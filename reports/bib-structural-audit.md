# Full Bib Audit (2026-02-23)

## Summary
- Bib sources loaded: 1
- Total entries: 771
- Unique keys: 771
- Total issues: 2
- Errors: 0
- Warnings: 0
- Info: 2

## Bib Sources
- `/Users/brettreynolds/Documents/LLM-CLI-projects/.house-style/references.bib`

## Field Style Snapshot
- Journal fields: `journal`=349, `journaltitle`=4, both=0
- Time fields: `year`=767, `date`=0, both=4
- Place fields: `address`=329, `location`=7, both=0

## Issues By Code
- `mixed_address_location_style`: 1
- `mixed_journal_field_style`: 1

## Issue Details
### `mixed_address_location_style` (1)
- [info] Mixed use of `address` (329) and `location` (7).
  Suggestion: Choose one location field style and normalize.

### `mixed_journal_field_style` (1)
- [info] Mixed use of `journal` (349) and `journaltitle` (4).
  Suggestion: Choose one field style for articles and normalize across the bibliography.
