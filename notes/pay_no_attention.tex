...
This conditioning isn't just passive tracking; it's an adaptive response to the communicative environment. \textcite{Noelle2018} show experimentally that systematicity evolves specifically when speakers must communicate about \emph{displaced} referents (things not present in the room) or in \emph{open} environments (where new referents constantly appear). Abstraction is a compression algorithm for working memory. When the environment gets too big to point at, the grammar gets systematic to handle the load.
