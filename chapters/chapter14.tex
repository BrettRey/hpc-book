...
The feeling of ungrammaticality isn't a simple signal. It has three registers, and they map onto the triadic architecture of Chapter~\ref{ch:the-category-zipper}.
...
When the coupling fails, all three registers fire: the jolt, the block, and the correction. The feeling of ungrammaticality is a chord.
...
In sum: Grammaticality itself is the structural property of the form--value coupling. What it's \emph{like} is the metacognitive readout of that coupling: silence when it works, a jolt when it fails, and a graded sense of \enquote{weirdness} or fluency when it's somewhere in between. The phenomenology has three registers~-- qualitative, reactive, and normative~-- and these correspond to the triadic structure of sign, object, and interpretant.
...
Syntactic satiation is schema drift in real time. The \enquote{hole} (the grammatical category) deforms to accommodate the repeated \enquote{peg} (the marginal sentence).

But when a constraint's categorical~-- as left-branch extraction is in English~-- no amount of exposure can change that. The resistance doesn't imply the structure is unimaginable; it implies the learner has detected a \term{conspicuous absence}. Fronting is productive in English; the *potential* for extraction is visible. But in this specific configuration, the community systematically avoids it (\textasciitilde3-sigma absence), using entrenched \term{workarounds} instead: pied-piping (\mention{whose book}), fused-head constructions (\mention{whose is this}), and the \mention{big mess} construction (\mention{how good a teacher}). \textcite{reynolds2026lbe} shows that this density of alternatives is precisely what makes the gap unsatiatable: the \enquote{satiation} path is blocked by the superior activation of the competitors. 
