...
\subsection{Firstness, Secondness, Thirdness}

Peirce's phenomenological categories offer a more systematic vocabulary for what we've been describing. \term{Firstness} is pure qualitative feel~-- the redness of red, the wrongness of wrong~-- prior to any reaction or interpretation. \term{Secondness} is resistance, brute facticity, the sense of something pushing back. \term{Thirdness} is mediation, rule, habit~-- what makes a correction meaningful rather than arbitrary.

The feeling of ungrammaticality involves all three. The qualitative \enquote{wrongness} is Firstness: a pre-reflective sense that something's off, the way a sour note feels sour before you know which note it is. The reactive resistance~-- the hesitation, the repair, the sense that you \emph{can't} say it that way~-- is Secondness: brute pushback from the system, the experience of being blocked. And the stabilized habit that makes the correction count as a correction~-- the fact that \mention{I'm twenty years old} is the right repair for \ungram\mention{I have twenty years}~-- is Thirdness: the rule-governed structure that gives the feeling its point.

This maps onto the triadic architecture of Chapter~\ref{ch:the-category-zipper}. The form is the sign-vehicle (Firstness as perceivable quality). The object is what's coupled (Secondness as brute constraint on what the form can mean). The interpretant is the inferential habit the coupling produces (Thirdness as mediated rule). When the coupling fails, all three registers fire: the qualitative jolt, the reactive block, and the normative correction. The feeling of ungrammaticality isn't a simple signal; it's a chord.

Peirce's own term for the irritation that forces inquiry was \enquote{doubt}~-- not skeptical doubt, but the felt disequilibrium that demands repair. Grammatical doubt works the same way. It's an irritant. It forces repair not because speakers follow rules consciously, but because the system won't settle until the coupling closes. The feeling is the immune system's first responder.

\bigskip

In sum: Grammaticality itself is the structural property of the form--value coupling. What it's \emph{like} is the metacognitive readout of that coupling: silence when it works, a jolt when it fails, and a graded sense of \enquote{weirdness} or fluency when it's somewhere in between. The phenomenology has three registers~-- qualitative (Firstness), reactive (Secondness), and normative (Thirdness)~-- and these correspond to the triadic structure of sign, object, and interpretant.
