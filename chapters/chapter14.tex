...
Most analyses of constructional data presuppose a distinction between grammatical and ungrammatical. The asterisk's the most common symbol in syntax, and one of the least likely to come with operating instructions. But what does the asterisk diagnose?
...
A note on what structural meaning isn't. It isn't semantic coherence (\mention{Colorless green ideas} shows grammaticality survives semantic absurdity). It isn't contextual felicity (a sentence can be grammatical and pragmatically infelicitous). It isn't whatever the hearer infers (inference runs on many channels; grammaticality's specifically the morphosyntactic one). Structural meaning's the contribution of form to interpretation~-- the \term{wiring diagram}, not the current flowing through it.
...
When something's wrong with the form--value coupling, speakers notice. They hesitate, repair, rephrase. The feeling's what triggers the correction. It's the immune response of the grammar. Like immune systems, it can develop allergies: harmless processing hiccups get treated as pathogens.
...
Coupling shapes processing; processing shapes detector output; detector output crosses threshold; threshold triggers repair; repair reshapes coupling. To keep this concrete, we can write the loop with five moving parts:
\begin{quote}
...
But here's the puzzle: if grammaticality \emph{is} the maintained coupling, and entrenchment's what maintains it, then \enquote{tuning the detector} and \enquote{shaping what counts as grammatical} can't be fully separated over historical time. At a single time-slice, they \emph{are} separable~-- detector output can misfire against a currently entrenched state, as illusions show. Across time, though, detector-guided repair and uptake reshape the coupling itself. The coupling tunes the detector; the detector, over time, tunes the coupling. That's a reflexive equilibrium dynamic, not a vicious circle.
...
These illusions are exactly analogous to visual illusions. The Müller-Lyer lines are the same length, but the perceptual system reports otherwise. The illusion doesn't show perception's broken; it shows perception is a mechanism with characteristic failure modes. Grammaticality illusions show the same point: detector, not oracle.
...
The form--value coupling's specifically morphosyntactic, and this predicts what won't trigger ungrammaticality. A phonetic error, like \mention{[\ipa{fɪlɔzəfi}]} for \mention{philosophy}, can misfire without being ungrammatical. A lexical error, like saying \mention{airport} when you meant \mention{train station}, can misfire without being ungrammatical. A semantic anomaly, like \mention{Colorless green ideas sleep furiously}, can misfire without being ungrammatical.
...
Each channel has its own detector, its own failure mode, and its own phenomenology. Grammaticality failure triggers the immune response we've been describing: hesitation, repair, the jolt of \enquote{wrongness}. Register mismatch triggers a different response: the raised eyebrow, the red ink on contractions, the sense that something's \enquote{off} without being \emph{ungrammatical}. The raised eyebrow is a detector too, but it rarely comes with calibration curves or inter-annotator agreement. Accent mismatch triggers identity attribution: \enquote{that's not how we say it here.} Lexical inappropriateness triggers precision failure: \enquote{that's not quite the right word.}
...
On this model, a prototypically grammatical sentence sits at the bottom of a deep basin. Perturb it slightly~-- change word order, shift stress, substitute a near-synonym~-- and the system tends to correct. A clearly ungrammatical sentence sits well outside the basin: small perturbations typically don't roll it back to stable ground.
...
This reframing has consequences. If grammaticality is basin-stability, then at least part of the apparent sharp boundary between well-formed and ill-formed can be an artifact of how we test. Gradient judgments aren't noise to be smoothed away; they're signal. The gradient isn't a defect in the evidence; it's the evidence under strain.
...
As a \term{mechanism}, it's a stabilizer for other HPCs. The category \term{noun} is maintained partly because nounhood is grammatical~-- because using a word in a noun slot, with noun morphology, in noun constructions, triggers no mismatch signal. Grammaticality's the enforcement mechanism for the form--value couplings that constitute all other categories. Grammaticality is maintained because it maintains those other couplings.
...
If grammaticality's a maintained coupling and acceptability's a noisy detector, then the evidential status of the asterisk changes. It's a signal from a measuring instrument~-- informative, but fallible, and calibrated to the community in which it was trained.


\section{Empirical signatures}
\label{sec:14:empirical}

If the feeling of ungrammaticality's a noisy detector~-- detector, not oracle~-- then laboratory studies should reveal its characteristic failure modes. Three bodies of evidence are consistent with this prediction.
...
Grammaticality's not a primitive. It's not a direct readout of structure. It's not possibility versus impossibility, but coupling versus mismatch: form and value, zipper teeth that mesh or don't. The feeling that signals mismatch is a noisy instrument~-- detector, not oracle~-- shaped by processing constraints and what a community has entrenched.
