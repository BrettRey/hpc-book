...
This is the \term{zipper} introduced in Chapter~\ref{ch:the-category-zipper}. Morphosyntax provides one set of teeth; structural meaning provides the other. Grammaticality is what it feels like when the zipper closes. An ungrammatical sentence is a zipper that won't mesh~-- teeth on one side that don't match teeth on the other.
...
\begin{enumerate}
    ...
    \item \textbf{Probability given the distribution}: The detector is Bayesian. A novel sentence is weighted by its likelihood under the distribution of attested forms~-- not just its distance from the nearest exemplar, but its probability given the entire pattern. Low-probability forms feel suspect even when grammatically licensed; high-probability forms feel natural even when they stretch the system.
\end{enumerate}
