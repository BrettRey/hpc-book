...
A caveat about evidential asymmetry: the quotative case rests on decades of variationist data~-- panel studies, apparent-time comparisons, cross-linguistic replication. The \mention{whose} case that follows is more speculative. The evidence is attestation data, not inter-speaker variation data; the framework predicts that such constructions should show high inter-speaker variation and sensitivity to discourse context, but we have not yet tested this prediction experimentally. A planned grammaticality-judgment study will address this gap. For now, the \mention{whose} case should be read as a \emph{prediction} of the HPC framework~-- a demonstration that the mechanisms \emph{should} operate here if the framework is correct~-- rather than a confirmation on par with quotatives.
...
The case studies in this chapter aren't meant to supersede the work they draw on: Remijsen's descriptive fieldwork on under-documented languages, Ferrara and Bell's functional analysis of quotative emergence, Bybee's usage-based account of frequency and entrenchment, Tagliamonte's variationist quantification of change in progress, Buchstaller's cross-variety typology, or Huddleston and Pullum's descriptive grammar with its attention to distributional and functional mechanisms. That work provides the data and often identifies the mechanisms~-- the primary documentation, the functional distinctions, the processing pressures, the apparent-time percentages, the distributional generalisations. The maintenance view provides a conceptual frame that makes the data cohere and explains why the mechanisms cluster.
