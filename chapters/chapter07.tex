...
\subsection{A template from biology}
\label{sec:7:biological-template}

If this sounds abstract, consider how immunologists talk about cell types. A macrophage is not defined by a single marker or function. It's picked out by a constellation: typical functions (phagocytosis, antigen presentation), marker profiles, morphologies, response tendencies, and developmental origins. None of these is strictly necessary and sufficient across all contexts. Sound familiar?

The stabilisation story is explicitly multi-level. Gene regulatory networks produce relatively robust ``attractor'' states -- configurations that the cell tends to settle into and return to after perturbation. Developmental lineage constraints make certain states easier to reach and maintain than others. The signalling environment -- local cytokines, tissue context -- pushes cells into different but related regions of the cluster. And there's functional feedback: what the cell does alters its microenvironment, which in turn sustains or shifts its state.

The key move is how immunologists handle variation. They're comfortable saying: \emph{same kind, different activation state}. A classically activated macrophage and an alternatively activated macrophage are both macrophages; the difference is which signals they've received and which region of the state space they occupy. Variation isn't embarrassing fuzziness to be explained away. It's expected, given the mechanisms.

This is the posture we want for linguistic categories. A noun in subject position and a noun in vocative position are both nouns; the difference is discourse context and which region of the distributional space the token occupies. The category is real because the stabilising story is real, not because every instance looks identical.

The mapping is almost direct:

\begin{itemize}
    \item Gene regulatory networks $\rightarrow$ cognitive architectures and processing biases
    \item Developmental lineage $\rightarrow$ acquisition pathways and learning biases
    \item Microenvironmental signalling $\rightarrow$ discourse context and pragmatic ecology
    \item Functional selection $\rightarrow$ communicative efficiency and social indexing
    \item Population structure $\rightarrow$ speech community networks and diffusion dynamics
    \item Attractor states $\rightarrow$ entrenched constructions and categories
\end{itemize}

What biologists have been doing for decades -- modelling how stable kinds emerge from multi-level causal processes without definitional essences -- is exactly what linguists need for grammatical categories.
...
Or consider an ongoing change: the rise of French \mention{on} in place of \mention{nous} for first-person plural. In spoken French, \mention{on} now appears in 90--98\% of contexts where \mention{nous} would be prescribed. Why?

At the level of processing, \mention{on} takes third-person singular conjugation -- simpler, faster. At the level of acquisition, children hear \mention{on} overwhelmingly; \mention{nous} is input-starved in casual registers. At the level of community norms, \mention{on} signals informality and solidarity where \mention{nous} sounds stiff or literary. At the level of transmission, each generation receives more \mention{on} and produces more \mention{on}, ratcheting the change. At the level of institutional pressure, schools and style guides resist -- but the resistance holds only in formal written registers, creating stable stratification rather than reversal.

Again, these aren't competing explanations. They're different angles on a single multiscalar process. And the interesting fact -- that \mention{nous} persists in formal writing while nearly vanishing from speech -- falls out of the mechanism profile: institutional pressure operates in written registers; it doesn't reach casual conversation. The register split is what you'd predict from the mechanism braid.
