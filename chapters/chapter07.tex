...
The lemma-concrete model~-- the one that treats each verb individually, without abstracting to aspect~-- predicts human behaviour better than the aspect-aware models. Under this AIC-based comparison, relative support for the aspect-aware models is extremely low.\footnote{AIC 3,891 for lemma-concrete versus 3,954 for aspect-concrete and 4,037 for aspect-abstract; evidence ratios in the $10^{13}$ to $10^{17}$ range~-- numbers so large that the model-comparison conclusion is statistically compelling.}
...
To make the mechanism/projectibility distinction concrete, we can use an \term{agent-based model} (ABM).\footnote{The Python scripts for the simulations in this book are available at \url{https://github.com/BrettRey/hpc-book/tree/main/code}.} If ABMs are unfamiliar: read this as a controlled cartoon. We specify a few local learning rules, let many interactions run, and ask what population-level pattern those rules can sustain.
