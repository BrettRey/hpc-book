...
The quotative case shows mechanisms maintaining a category from above~-- from the functional and social pressures that keep the clustering clustered. A second case shows mechanisms maintaining a construction from below~-- from the parsing operations that make it processable.
...
But the gap may be illusory. \citet{reynolds2024whose} documents attestations spanning seven centuries, from Middle English manuscripts to contemporary academic prose: \mention{a friend of whose had told us of the accident}; \mention{those whose mental faculties are intact versus those whose are not}. The construction is rare~-- perhaps once in millions of words~-- but attested in edited sources across registers.\footnote{The grammaticality judgments remain disputed. \textcite{reynolds2024whose} is a preprint, and native-speaker intuitions vary. The epistemic status is accordingly tentative: suggestive evidence that the mechanism is there, not conclusive proof.}

What Hankamer and Postal diagnosed as syntactic prohibition may instead be pragmatic licensing. The construction requires specific information-structural conditions~-- contrastive parallelism, deictic anchoring, structural integration~-- that ensure the elided possessum remains recoverable. Consider:

\begin{quote}
\mention{I knew someone whose greatest love affair was with objects, another whose was with books, and a third whose was with ideas.}
\end{quote}

The parallel structure establishes the possessum type (\mention{love affair}) in the first clause, licensing its ellipsis in the subsequent clauses. Without such licensing~-- without contrastive focus, explicit parallelism, or deictic anchoring~-- the construction crashes. Hankamer and Postal's examples lacked all three.

\subsection{Same mechanism, different visibility}

Independent relative \mention{whose} is a filler-gap construction. \mention{Whose} fills a gap in subject or complement position: \mention{a woman whose \_\_ was straight}. The same parsing mechanism that Boguraev et al. showed transfers across construction types is what makes \mention{whose}-stranding possible. The mechanism projects.

What looked like a syntactic gap was actually the mechanism operating under unusually stringent pragmatic conditions. The filler-gap parser is there; discourse accessibility determines when it can succeed.

Cross-linguistic evidence supports this. German and Japanese~-- despite radically different syntactic structures~-- show identical information-structural constraints on independent possessive relatives. The constraints are not language-specific accidents; they're consequences of how the parsing mechanism interacts with discourse accessibility. The mechanism is universal; its licensing conditions are pragmatic.

\subsection{Reciprocal maintenance, again}

The filler-gap case shows the same reciprocal structure as quotatives. The parsing mechanism maintains the constructions (by making them processable); the constructions maintain the mechanism (by exercising it repeatedly). Each relative clause, each interrogative, each cleft is input to the cognitive architecture that sustains filler-gap processing. The mechanism is real because it's kept alive.
...
A reminder of the pluralist posture established in Chapter~\ref{ch:kinds-without-essences}.
