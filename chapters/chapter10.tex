...
%--- --- --- --- --- --- --- --- --- --- --- --- --- --- --- --- --
\section{The puzzle of the definite article}
\label{sec:10:hook}
%--- --- --- --- --- --- --- --- --- --- --- --- --- --- --- --- --
...
Each uses \mention{the}, but none involves the standard uniqueness or familiarity. These aren't marginal exceptions; they're high-frequency, fully productive patterns \citep{birnerward1994}. A theory of \mention{the} that treats them as anomalies risks missing the systematicity of the pattern.
...
On the semantic side sits the \term{definiteness cluster}: the interpretive properties that make a referent identifiable. These include familiarity (the referent is discourse-old), uniqueness (there's only one candidate in the relevant domain), and identifiability (the hearer can pick it out).\footnote{Specificity~-- whether the speaker has a particular referent in mind~-- is sometimes included, but it crosscuts the definite/indefinite distinction: indefinites can be specific (\mention{I'm looking for a book~-- it's on my shelf}) or non-specific (\mention{I'm looking for a book~-- any book}). Specificity tracks speaker knowledge; definiteness tracks hearer knowledge.} When all three align, we have a prototypically definite referent~-- \mention{the cat we discussed yesterday}. Anaphoric recoverability~-- the capacity to be picked up by a pronoun~-- is a downstream discourse affordance, not a core property.
...
The morphosyntactic cluster in English is defined by distributional diagnostics. These are grammatical tests, not semantic ones ~-- they target structural behaviour, not meaning. Four diagnostics converge to define the cluster.
...
\begin{exe}
    \ex \label{ex:there-bad} \ungram{\textit{There is the key/my key on the table.}}
...
This selectional restriction doesn't yield to prosodic manipulation.
...
\begin{table}[htbp]
\centering
\caption{Diagnostic profile for English determiners. \checkmark\ = exhibits form-cluster behaviour; ~-- = does not. Diagnostics: \mention{there} = resists existential \mention{there} under neutral prosody; Partitive = licenses partitive complement; Hosting = hosts nonrestrictive modification; \mention{one} = resists \mention{one}-substitution.}
...
\mention{this/that}\textsuperscript{*} & \checkmark & \checkmark & \checkmark & \checkmark \\
Genitives & \checkmark & \checkmark & \checkmark & \checkmark \\
\mention{each/every} & \checkmark & ~-- & ~-- & \checkmark \\
\mention{a/an} & ~-- & ~-- & ~-- & ~-- \\
\mention{some} & ~-- & ~-- & ~-- & ~-- \\
\bottomrule
\multicolumn{5}{l}{\footnotesize *In deictic uses; narrative-presentational \mention{this} (\mention{There was this guy...}) is a} \\
\multicolumn{5}{l}{\footnotesize derived-function construction that recruits demonstrative morphology for non-deictic use.}
\end{tabular}
\end{table}
...
These mechanisms interlock. Grammaticalization creates the bundle; acquisition transmits it; alignment maintains it in real time; prestige shapes which variants survive; transmission filters for stability. Metalinguistic feedback adds another layer: grammarians' labels and diagnostics can feed back into pedagogy and prescriptive norms, potentially stabilising or destabilising particular constructions. Perturb any mechanism, and the equilibrium shifts~-- but the cluster persists because the other mechanisms compensate.
...
This is a derived function. The article's distributional behaviour was inherited from the demonstrative source~-- a side effect of grammaticalization, not what the article is \emph{for}. Once that side effect stabilized, speakers could recruit it for institutionalized activity frames. But weak definites aren't merely fossils: the construction is productive. We \mention{take the Uber}, \mention{check the app}, \mention{get on the WiFi}. New forms enter the pattern whenever a stereotypical activity arises. What persists is the frame~-- verb + \mention{the} + role-denoting noun~-- recruiting form-cluster membership for activity-type semantics.
...
Proper names look like a failure of marking. \mention{Kim} and \mention{London} are semantically definite~-- they identify unique, familiar referents~-- but they lack form-cluster marking in English.
...
The conflation has a history. When Russell formalized \mention{the} as a quantifier encoding uniqueness, the article's morphosyntactic behaviour was not the object of analysis~-- the semantic contribution was. Later work refined the semantics but inherited the assumption: \enquote{definiteness} named both the meaning and the marking. The slide was reinforced from other directions too~-- grammatical description, language pedagogy, typological comparison~-- each treating form and function as a single package. The term did double duty, and its equivocation became invisible. Every weak definite that didn't fit the semantic story became an exception to be explained rather than evidence that two categories had been conflated.
...
The term is ugly~-- methodologically ugly. Its unfamiliarity forces a break with the assumption that form and function are identical. When you say \mention{deital}, you cannot accidentally mean \mention{definite}. The conceptual separation becomes lexicalized. This is a terminological intervention, and it has costs: new labels require uptake. But the alternative~-- continued equivocation on \mention{definiteness}~-- has higher costs still.
...
The next case study pushes further. Lexical categories ~-- noun, verb, adjective ~-- show even more complex patterns. The noun/verb contrast is strikingly stable cross-linguistically; adjectives vary dramatically in both inventory and behaviour. If these are HPC kinds, the mechanisms maintaining them have to vary as well. Some may be deep and universal (cognitive, developmental); others may be local and reconfigurable (constructional, sociolinguistic).
