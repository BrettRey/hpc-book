...
\mention{Quickly}. \mention{Absolutely}. \mention{However}. \mention{Frankly}. \mention{Moreover}. \mention{Somewhat}. \mention{Yesterday}. \mention{Downstairs}. \mention{Therefore}. \mention{Certainly}.

What do these words have in common? The dictionary says they're adverbs. But what does that mean? They modify different things: \mention{quickly} modifies manner, \mention{very} modifies degree, \mention{frankly} signals speaker stance, \mention{however} organises discourse, \mention{yesterday} locates time, \mention{therefore} marks inference. Their distributions overlap only partially. Their semantic contributions share almost nothing. The label \mention{adverb} gathers them like socks in a drawer---not by what they are, but by what they aren't. They're not nouns; they're not verbs; they're not adjectives. What remains is the bin.

This chapter is a dissection. We will find what holds together and what doesn't.

I won't catalog every word class. The goal is to ask a different question: what makes a category cohere---or fail to cohere? We'll examine four major classes (nouns, verbs, adjectives, adverbs) and one apparent class (pronouns) that turns out to be several. The closed functional categories---determiners, prepositions, conjunctions---are a different story, better left to typological specialists \citep{haspelmath2010}. Our business is with the mechanisms, and the mechanisms are clearest where they succeed spectacularly (nouns and verbs), fail instructively (adverbs), or produce an illusion that rewards closer inspection (pronouns).
...
Start with failure. If word classes are \abbr{HPC} kinds maintained by mechanisms, then a class with no unified maintenance regime should fall apart under scrutiny. Adverbs are that class.
...
\item \textbf{Value} (\mention{good}, \mention{bad}): highly evaluative, prone to grammaticalisation, unstable clustering.
\end{itemize}
...
\begin{exe}
    \ex \label{ex:det-resist-today} \ungram{the today} / \ungram{a today}
    \ex \label{ex:det-resist-rel} \ungram{the person the who called}
    \ex \label{ex:det-resist-int} \ungram{The who called?}
\end{exe}
...
\begin{exe}
    \ex \label{ex:focus-int} \textit{Exactly who called?}
    \ex \label{ex:focus-rel} \ungram{the person exactly who called}
\end{exe}
...
\begin{exe}
    \ex \label{ex:fused} \textit{I'll help whoever needs it.}
    \ex \label{ex:fused-focus} \textit{?I'll help exactly whoever needs it.}
\end{exe}
...
\textbf{Nouns and verbs} are tight \abbr{HPC} kinds: robust cross-linguistic presence, converging mechanisms at cognitive, discourse, acquisition, and transmission scales. They are the skeleton---the load-bearing architecture without which a language cannot function.
