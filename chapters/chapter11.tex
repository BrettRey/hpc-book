...
The \abbr{HPC} answer is causal. Nouns and verbs are stable because they are maintained by converging mechanisms at multiple scales. In the triadic terms of Chapter~\ref{ch:the-category-zipper}:

\begin{itemize}
    \item \textbf{Form}: the distributional profile~-- argument positions, determiners, inflection, agreement patterns.
    \item \textbf{Object}: the conceptual type~-- things for nouns, events for verbs.
    \item \textbf{Interpretant}: the inferential habit~-- what argument-structure expectations, referential functions, and predicational roles the category licenses.
\end{itemize}

What makes nouns and verbs so robust is that all three terms are tightly aligned. When you hear nominal morphology, you expect thing-reference; when you construe a thing, you reach for a noun. The interpretant pulls form and object together so reliably that the mechanism disappears. Adverbs lack this alignment: multiple forms, heterogeneous objects, no unified interpretant.
