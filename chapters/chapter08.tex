...
The maintenance view gives clear guidance when you diagnose a fat class: decompose. \term{Adverb} isn't an HPC. But the genuine clusters lurking inside it~-- the cluster around \mention{quickly}, the cluster around \mention{very}, the cluster around \mention{thus}, each maintained by its own braid of mechanisms~-- may be kinds where the umbrella isn't. The lump is a taxonomic fiction; the components are causally real. This doesn't mean abolishing the term~-- \term{adverb} remains useful for some purposes~-- but it means recognizing that the term names an epistemic kind, not a natural kind. When you're doing HPC analysis, look below the lump.
...
\label{ex:lbe-b}
\ungram\mention{Whose did you read book?} (Left Branch Extraction)\label{ex:lbe-b}
\z
\z
...
Why is the gap so stable? \textcite{Reynolds2026} argues it's maintained by the density of \term{competitor strategies} (what we might call \enquote{workarounds}). If you want to question a possessor, you have high-frequency, fully entrenched options:
...
\subsection{Re-unified coupling}
