...
The diagnostics developed in this chapter can be synthesized into a reusable template. For any candidate category $C$, the audit proceeds as follows:\footnote{Part III applies this template to countability (Chapter~\ref{ch:countability}), definiteness/deitality (Chapter~\ref{ch:definiteness-and-deitality}), and word classes (Chapter~\ref{ch:lexical-categories}). Each chapter ends with an abbreviated version of this audit.}
...
6. Failure-mode gate & \textbf{Thin:} weak clustering or weak stabilizer story. \textbf{Fat:} heterogeneous subclusters or heterogeneous stabilizers with interest-dependent inductions. \textbf{Negative:} primarily defined by absence or contrast-set complement. \\[6pt]
...
\noindent The audit is not a checklist to be completed once and filed away. It is a diagnostic instrument to be re-run when predictions fail, when the scope shifts, or when new perturbation evidence becomes available. The goal is not to certify categories but to calibrate confidence in them~-- and to make explicit what would change that confidence.
