...
And this is where we can finally answer Rodney Huddleston's puzzle from §\ref{sec:1:opening}. His 3~AM email about \mention{otherwise}~-- \enquote{Its classification is quite a puzzle}~-- wasn't a failure of scholarship; it was a field report from inside the wastebasket.

\mention{Otherwise} is a manner adverb: \mention{think otherwise}. \mention{Otherwise} is a predicative adjective: \mention{the truth is quite otherwise}. \mention{Otherwise} is the hinge in a construction: \mention{the correctness or otherwise of the proposal}.

So no: Huddleston wasn't failing to find the right label. The labels were failing to fit what he saw. \mention{Otherwise} doesn't live in one subcluster; it is recruited across several, stabilised by different mechanisms in different contexts. The puzzle wasn't the word; it was the expectation that one label should cover all its jobs.
