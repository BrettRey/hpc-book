...
But not every spinning top is the same. Some wobble for a moment and fall; some were never really spinning at all~-- just labels we applied to patterns that happened to be there. Section~\ref{sec:4:heterogeneity} previewed three ways a category can miss: thin, fat, or negative. This chapter develops the diagnostics. The answer turns out to be that the ways a category can miss tell us something about what it takes to be a genuine kind.
...
\midrule
Thin & Fail (insufficient clustering) & Fail (no stabilisers) & \textit{Whose}-stranding \\
...
This pattern exists. \citet{radford2012} document it in live broadcast speech: roughly 15 copying tokens and 67 pruning tokens in \textasciitilde300 hours of recording. It is reproducible: such forms often pass without overt repair. But it is thin.

The frequency is vanishing~-- too rare to be plausibly acquired as a \emph{target} construction. There is no social indexing: copying/pruning signals nothing about identity, register, or stance. There is no functional niche: either pied-piping or stranding serves the same communicative end without redundancy or omission.
...
A \term{fat} class is a label that lumps distinct causal clusters into a single bin. The analyst treats them as one~-- usually for \enquote{wastebasket} reasons, where the class is defined residually: \enquote{everything that isn't X goes here.} But the mechanisms maintaining the subgroups are radically different. Fat classes miss the projectibility diagnostic: learning one subclass doesn't let you predict others.
...
And this is where we can finally return to Huddleston's puzzle from §\ref{sec:1:opening}. His 3~AM email about \mention{otherwise}~-- \enquote{Its classification is quite a puzzle}~-- wasn't a failure of scholarship; it was a report that the usual diagnostics do not converge.

Start with the uses that look easy. In \mention{think otherwise}, \mention{otherwise} sits where we expect adverbs to sit, and it has the familiar meaning \enquote{in another way / to a different effect}. But even here, the comfort is partly illusory. English \mention{think} readily takes content-like complements in reduced form (\mention{think that}, \mention{think this}, \mention{think so}, \mention{think not}), and \mention{otherwise} is at least a plausible member of that family: not merely a manner modifier, but a compact alternative to \enquote{that things are otherwise}. In other words, the first use is already perched on the boundary between modifier and complement.

Now consider the predicative case: \mention{the truth is quite otherwise}. Whatever category \mention{otherwise} belongs to, it is functioning as predicative complement here. The stress point is not the function but the category fit. Degree modification by \mention{quite} and the idiomatic force \enquote{contrary to what is assumed} pull it toward adjective-like behaviour; its kinship with \mention{otherwise} in \mention{think otherwise} pulls it back toward the modifier inventory. The diagnostics cross-cut.

The two examples Huddleston explicitly flagged make the conflict sharp. In \mention{this suggests otherwise}, \mention{otherwise} behaves like a content complement: the construction invites something proposition-sized (\mention{this suggests that...}), and \mention{otherwise} appears to do that job in a reduced, anaphoric way. And in \mention{the correctness or otherwise of the proposal}, \mention{otherwise} is recruited into a coordination that functions inside a nominal: it behaves like the conventional way English expresses \enquote{whether X holds or the contrary holds}. Neither environment is a natural habitat for an ordinary VP adjunct, yet both are robust, non-marginal uses.
...
The projectibility test delivers a clear negative. The category \term{adverb} doesn't support cross-subclass generalisation. What you learn about one subtype tells you nothing reliable about another. This is diagnostic of a fat class: the label groups items that were never maintained by shared mechanisms.
...
What do they share? The standard answer is negative: they lack tense marking, they lack agreement (mostly), they can't head a main clause (mostly). They're grouped by absence.
...
But \enquote{English Grammar} is just the island where they all live. It's the \term{constructicon}~-- the population of constructional species inhabiting a speech community. 
...
\begin{itemize}
    \item \term{Noun} is likely a fat class (an umbrella for many specific nominal constructions).
...
\citet{haspelmath2010} makes a related point with his distinction between \term{comparative concepts} and \term{descriptive categories}. Comparative concepts are analyst's tools~-- definitions constructed for cross-linguistic comparison. Descriptive categories are language-specific~-- the actual clusters maintained in particular speech communities. The maintenance view explains \emph{why} these must be kept separate. Comparative concepts are epistemic kinds: useful for organising comparison, but not tracked by mechanisms. Descriptive categories may be HPC kinds~-- but only if they pass the diagnostics in that particular language. Cross-linguistic \enquote{sameness} is a claim about convergent mechanisms, not shared essence.

Consider evidentiality. In Quechua, evidential markers are obligatory on tensed verbs: speakers must encode whether information comes from direct experience, inference, or report. In Turkish, the suffix \textit{-mIş} marks non-first-hand information~-- inference or hearsay~-- but leaves direct evidence unmarked. In English, evidentiality is optional and expressed adverbially: \mention{apparently}, \mention{reportedly}, \mention{obviously}. Are these \enquote{the same category}?

The maintenance view says: apply the diagnostics. In Quechua, the evidential paradigm is maintained by obligatory morphological slot-filling, paradigm pressure, and early acquisition as part of the verbal template~-- robust homeostasis. In Turkish, \textit{-mIş} is maintained by similar mechanisms, but the asymmetry (marking inference but not direct evidence) suggests a different functional niche. In English, the adverbials are maintained by pragmatic conventions and discourse-level alignment~-- weaker mechanisms, scattered across the lexicon rather than concentrated in a morphological paradigm.

These are three different HPCs. Calling them all \enquote{evidentiality} conflates distinct causal structures under a shared label~-- the fat-category problem at the cross-linguistic scale. The label is useful for comparison (\enquote{how do languages mark information source?}), but misleading for mechanism analysis. The genuine kinds are the language-specific clusters, not the typological umbrella.
