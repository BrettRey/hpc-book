...
And this is where we can finally return to Huddleston's puzzle from §\ref{sec:1:opening}. His 3~AM email about \mention{otherwise}~-- \enquote{Its classification is quite a puzzle}~-- wasn't a failure of scholarship; it was a field report from inside the wastebasket~-- a report that the usual diagnostics do not converge.
...
What do these clause types share? The standard answer is negative: their heads lack tense inflection, they typically lack overt subjects, they can't stand alone as main clauses (mostly). They're grouped by absence.
...
So the framework does not hand you a privileged unit in advance. It hands you a discipline: propose a unit, apply the diagnostics, and let the result tell you whether to decompose, reframe, or set the label aside. In practice, the mechanisms are often easiest to see at local grains~-- phoneme contrasts, lexemes, specific constructions, sometimes families or paradigms when stabilisers genuinely operate at that scale. What is rarely a kind is \enquote{English Grammar} as a unit. That is the fauna: an ecosystem of many kinds maintained by many mechanisms.
...
That said, some comparative concepts may earn \term{naturalised} status when independent languages converge on similar patterns through similar mechanisms~-- the way camera eyes evolved independently in vertebrates and cephalopods. Naturalization is defeasible: it requires explicit mappings from comparanda to realizations, graded weights that track how strongly each language-specific form realizes the target, and a firewall that diagnoses semantic targets independently of their morphosyntactic expression. Without this discipline, we conflate labels with kinds.
...
The label \enquote{evidentiality} conflates the semantic target with the morphosyntactic system. The target may naturalise: information-source marking is a stable discourse need, and some subtargets (reportative vs. inferential) may form genuine semantic clusters. But \enquote{the category evidentiality} as a cross-linguistic morphosyntactic kind is probably too fat~-- it bundles obligatory paradigms, optional suffixes, and scattered adverbs under one umbrella, masking the distinct mechanisms maintaining each. The genuine HPCs are the language-internal systems: Quechua's paradigm, Turkish's suffix. English has no comparable paradigm.
