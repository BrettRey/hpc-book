...
Matthew Slater offers a refinement that sidesteps the homeostasis requirement while preserving what Boyd got right \\citep{slater2015}. His \\term{stable property cluster} (SPC) account requires only that properties cluster reliably over time~-- not that a single mechanism actively restores equilibrium. Multiple unknown mechanisms may ensure stability; what matters is the observable clustering. This relaxes Boyd's requirement without abandoning the core insight: kinds are real because properties stay bundled, not because we can identify a unique causal ground.
