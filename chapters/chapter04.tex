...
The same logic applies to grammatical categories. Nouns share properties~-- they head noun phrases, take determiners, inflect for number, denote entities~-- not because these properties define \term{noun} but because the mechanisms of acquisition, entrenchment, and transmission keep them bundled. The properties vary; not every noun has all of them. But they cluster reliably enough that learning the category \term{noun} tells you something about how unfamiliar nouns will behave.
...
Table~\ref{tab:parallel-mechanisms} sketches the correspondences. Not every detail maps, and that's fine. The claim isn't that language \emph{is} biology but that the same style of explanation works: mechanism-based, historical, non-essentialist. What maintains the clustering matters more than what defines the category.
...
Children acquire grammatical categories without being taught definitions. No parent explains that nouns are words that can be preceded by determiners and followed by plural suffixes. Yet children converge on the same categories their parents have, reliably, across cultures, in just a few years.
...
The evidence is strong. Distributional learning~-- tracking which words occur in which contexts~-- is sufficient to induce grammatical categories from raw input \citep{redington1998,mintz2003}. Infants are sensitive to these distributions from early in the first year \citep{gomez2000}. The categories children induce match adult categories not because children have innate category templates but because the same distributional patterns that define adult categories are present in child-directed speech.
...
Entrenchment maintains categories by anchoring them. The most frequent nouns~-- \term{thing}, \term{time}, \term{way}, \term{people}~-- are processed automatically, without conscious categorisation. They're the bedrock of the category, the items that every speaker agrees on, the prototypes around which less frequent items cluster. Change the behaviour of \term{thing} and you change what \term{noun} means. But \term{thing} is so entrenched that changing it is nearly impossible~-- it would require shifting the habits of every English speaker simultaneously.

High-frequency items also grammaticalise faster. The path from lexical verb to auxiliary~-- \term{will}, \term{going to}, \term{have to}~-- runs through frequent, semantically general items. Low-frequency items don't grammaticalise because they're not processed often enough to undergo the phonetic reduction and semantic bleaching that grammaticalisation requires. The category \term{auxiliary} exists because entrenchment creates a basin of attraction: items that enter the basin get pulled toward the prototype; items outside it stay lexical.
...
Alignment maintains categories by enforcing community-wide coherence. If I use \term{impact} as a verb and you accommodate, we've both reinforced the verb use. If you resist~-- if you pointedly use \term{affect} instead~-- you've exerted pressure against the innovation. Either way, the interaction shapes subsequent usage. Multiply this by millions of conversations and you get a distributed mechanism for maintaining shared norms.
...
Categories exist because they do something. Nouns package referents for tracking across discourse. Verbs encode events and their participants. Determiners signal identifiability. The functions aren't arbitrary~-- they reflect communicative needs that any language must meet.
...
Where functional pressure is strong, categories are stable. The noun--verb distinction exists in every documented language because the functions nouns and verbs serve are universally needed. Where functional pressure is weak, categories vary. Not all languages distinguish adjectives from nouns; not all languages have adverbs as a distinct category. The functions these categories serve can be performed by other means~-- nouns can modify nouns; verbs can modify verbs~-- so the pressure to maintain them as separate kinds is lower.
...
HPC doesn't claim that all categories are the same. Some grammatical categories might be more essentialist than others; some might be barely maintained at all. The framework expects heterogeneity.

Consider the contrast between closed-class and open-class items. Closed-class categories~-- determiners, complementisers, auxiliaries~-- are small, stable, and learnable as lists. A child can memorise every English determiner; there are only a dozen or so. The mechanisms maintaining these categories operate differently than for open classes. Entrenchment is nearly total: every member is high-frequency. Variation is minimal: speakers agree on what the determiners are. The categories look almost essentialist~-- not because they have definitions but because the mechanisms maintain them so tightly that they behave as if defined.

Open-class categories~-- nouns, verbs, adjectives~-- are large, shifting, and impossible to list exhaustively. New items enter constantly; old items drift or exit. The mechanisms operate statistically, across thousands of items, with substantial variation at the margins. These are paradigmatic HPC kinds: property clusters with fuzzy boundaries, maintained by mechanisms that shape the distribution without determining any individual case.

Phonological categories might differ from syntactic ones. Phonemes are maintained by articulatory and perceptual mechanisms that are partly physiological~-- the boundaries of phoneme categories reflect facts about the human vocal tract and auditory system \citep{ohala1990}. Syntactic categories lack this physiological grounding; they're maintained entirely by learning and use. The prediction is that phonological categories should be more stable cross-linguistically (because the physiology is shared) while syntactic categories should vary more (because learning and use can differ).

The framework doesn't need all categories to be HPC kinds. Some categories might be thin~-- maintained by mechanisms too weak to produce robust clustering. Some might be thick~-- maintained by mechanisms so strong that they behave like essences. Some might be negative~-- not maintained at all, mere taxonomic conveniences that don't support induction. Part of the work, going forward, is identifying which grammatical categories are which. Chapter~\ref{ch:failure-modes} develops criteria for diagnosis.


\section{How determinacy survives}
\label{sec:4:determinacy}

A worry hangs over the HPC picture: if categories don't have definitions, how can category membership be determinate? Isn't this just nominalism with extra steps~-- the view that categories are fictions, not facts?
...
Where does indeterminacy arise? Where the causal pressures are genuinely mixed or transitional. \term{Fun} is a hard case not because we lack a definition to apply but because the mechanisms have been pulling in different directions: it was acquired as a noun but is being used increasingly like an adjective, with entrenchment patterns that are genuinely intermediate. The indeterminacy is real, but it's localised~-- a fact about this item in this transitional state~-- not a global fuzziness that infects the whole category.

Compare: is Pluto a planet? The question feels unanswerable, and for good reason. \term{Planet} was maintained by mechanisms~-- observation, naming, teaching~-- that produced a cluster of paradigm cases: Mercury through Neptune. Pluto was marginal from the start~-- small, oddly orbited, recently discovered. When the mechanisms shifted~-- when new objects were found that made the existing cluster incoherent~-- Pluto's status became indeterminate. But Mars's status didn't. The indeterminacy was localised to the boundary cases, where the mechanisms were weakest.
...
Chapter~\ref{ch:essentialism} exonerated Aristotle from the cartoon version of essentialism~-- the view that categories have crisp definitions consisting of necessary and sufficient conditions. Real Aristotle was subtler. His essences weren't arbitrary definitions; they were supposed to be explanatory. The form of a thing explained its behaviour; the essence was what made the thing the kind of thing it was.
...
But that's a feature, not a bug. Grammatical categories are inherently relational~-- defined by distribution, opposition, system role. An account that makes them intrinsic would be false to what they are. HPC captures their relational nature while preserving their reality. The categories are real \emph{because} they're relational~-- maintained by relations that keep properties clustered.
...
Chapter~\ref{ch:discrete-from-continuous} takes up this problem. The answer will involve scale-dependent tolerance, hyperreal formalisations, and a picture of category boundaries that are sharp but located at inaccessible distances. Dynamic discreteness, I'll call it~-- discreteness that emerges from the interaction of structure and mechanism.

But the foundation is now in place. Categories are HPC kinds. They're real because they're maintained. What remains is to understand the geometry of the basins they occupy~-- and for that, we need to talk about how discrete structure emerges from continuous substrates.


\section*{References for Chapter~\ref{ch:kinds-without-essences}}

% Note: These will be integrated into the main bibliography via BibTeX

\begin{refsection}

% The actual bibliography will be generated from the .bib file
% The following are the BibTeX entries needed for this chapter
