...
\begin{quote}
\textbf{HPC\ixsq{HPC} commitment:} A category is real when (a) it is associated with a robust cluster of properties, and (b) there exist processes that tend to produce and stabilize that cluster across the relevant range of contexts, yielding projectible generalisations.
\end{quote}
...
The evidence is strong. Distributional learning~-- tracking which words occur in which contexts~-- is sufficient to induce grammatical categories from raw input \ixnq{Redington, Martin}\ixnq{Chater, Nick}\ixnq{Finch, Steven}\ixnq{Mintz, Toben H.}\ixnq{Piantadosi, Steven T.}\ixnq{Kallini, Julie}\ixnq{Papadimitriou, Isabel}\ixnq{Futrell, Richard}\ixnq{Mahowald, Kyle}\ixnq{Potts, Christopher}\citep{redington1998,mintz2003,piantadosi2024,kallini2024}. Infants are sensitive to these distributions from early in the first year \ixnq{Gómez, Rebecca L.}\ixnq{Gerken, LouAnn}\citep{gomez2000}. The categories children induce match adult categories not because children have innate category templates but because the same distributional patterns that define adult categories are present in child-directed speech.
...
\section{HPC\ixsq{HPC} kinds aren't natural kinds}
...
Nicaraguan Sign Language\ixgq{Nicaraguan Sign Language (NSL)} (NSL\ixgq{Nicaraguan Sign Language (NSL)}; Spanish\ixgq{Spanish} acronym ISN\ixgq{Nicaraguan Sign Language (ISN)}) offers near-experimental evidence. NSL\ixgq{Nicaraguan Sign Language (NSL)} emerged in the late 1970s when deaf children were brought together in Managuan schools; no prior sign language was available as a model. Within two generations, NSL\ixgq{Nicaraguan Sign Language (NSL)} developed combinatorial phonology~-- discreteness, duality of patterning, minimal pairs~-- from holistic gestures \ixnq{Senghas, Ann}\ixnq{Kita, Sotaro}\ixnq{Özyürek, Asli}\citep{senghas2004,senghas2005}. The mechanisms were visible in real time: children segmented continuous gestures into discrete components, regularized variation, imposed categorical structure. Phonological organization wasn't inherited; it was constructed by the same pressures that maintain it elsewhere.
