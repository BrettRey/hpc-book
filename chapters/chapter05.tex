...
Nonstandard analysis adds infinitesimals to the reals~-- quantities greater than zero but smaller than any positive real number \citep{Goldblatt1998LecturesHyperreals}. Robinson's construction is rigorous, but we don't need its details. Here infinitesimals are just a way to make \emph{negligible relative to the current scale} precise without choosing an arbitrary finite cutoff. I am not claiming speakers compute with hypernaturals; the hyperreals are a modelling device, like calculus in physics. The full application to vagueness is developed in \citet{reynolds2025sorites}.
