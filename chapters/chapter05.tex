...
Linguistic categories look discrete: a word is a noun or it isn't, a sentence is grammatical or it isn't, a sound is /ɪ/ or /ɛ/ in the \mention{pin}/\mention{pen} contrast for the dialects that maintain it. But the properties underlying those categories are continuous: frequency distributions shade smoothly from one pattern to another, acoustic cues vary continuously (vowel formants don't come in bins), semantic features don't come with sharp edges. How do discrete categories emerge from continuous substrates?

Two familiar answers, each half right. The essentialist gets discreteness but loses reality~-- definitions that don't exist can't ground categories that do. The prototype theorist accepts the reality but loses the discreteness~-- why don't categories blur into each other over time? A third option~-- \citeauthor{Ambridge2020}'s radical exemplar model~-- goes further: categories aren't stored at all, just emergent from analogising across tokens. This gets the mechanism right but draws the wrong conclusion; Chapter~\ref{ch:kinds-without-essences} argued that emergent patterns maintained by reliable mechanisms \emph{are} real kinds. We need the fourth option: an account of how mechanisms operating on continuous substrates produce categorical outcomes. Call this \term{dynamic discreteness}~-- discreteness that is achieved, not given.
...
This is suggestive. If physical systems can exhibit discrete, stable structure without essences, perhaps linguistic categories can too. But the analogy needs to be more than suggestive. We need to say precisely what it is about the dynamics that produces discreteness, and we need to show that something analogous operates in language.
...
\textbf{[TODO: The basin metaphor here emphasizes \emph{diachronic} dynamics (how categories shift over time). But context also warps the terrain \emph{synchronically}~-- moment-to-moment, as in phoneme perception. The same word may be perceived as more noun-ish or adjective-ish depending on the syntactic frame. Not a static watershed but a terrain that flexes with context. See Ch~13 notes for development.]}
