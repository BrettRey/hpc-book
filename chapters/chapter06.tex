...
The HPC alternative makes a different prediction. On this view, category membership isn't maintained by definitions but by mechanisms~-- acquisition patterns, entrenchment through use, lexical specificity. Learners don't learn that perfective ``means'' bounded completion and apply that knowledge; they extract cue-outcome associations from input and let those associations drive behaviour. The mechanisms that maintain the category are distributional, not semantic. If HPC is right, a model that captures distributional learning should outperform one that applies semantic definitions. (We'll unpack these mechanisms fully in Chapter~\ref{ch:homeostasis}; for now, the key point is that the prediction is testable.)
...
This is why the aspect example matters. \term{Perfective} as traditionally defined~-- bounded, complete, telic~-- is less projectible than expected because those semantic properties, while correlated with aspectual usage, aren't produced by the mechanisms that maintain the aspectual system. The mechanisms are lexical bias + tense conditioning, not semantic boundedness. For most Polish verbs, there's nothing to choose: about 90\% strongly prefer one aspect, appearing in their preferred form over 90\% of the time \citep{divjak2025learnability}. Entrenchment has made the ``choice'' for them. Project from the mechanisms and you succeed; project from the definitions and you're predicting imperfective while Polish speakers are saying perfective.
