...
% TODO: Consider figure showing NPI "uniform label" vs "jagged reality" of licensing patterns
...
% TODO: Consider figure showing island/volcano distinction: volcano (historical cause) vs coral/roots (current maintenance)
...
\begin{description}
\item[Testing projectibility] To test whether a category is projectible, train a model on the proposed characterisation and measure its predictive success. If a semantic definition (e.g.\ boundedness for aspect) predicts usage poorly, try a mechanism-based alternative (e.g.\ lemma-specific cue structure). Compare. If the mechanism-based model wins, the definition has mislocated the causal structure. If neither wins, the category may not be maintained by consistent mechanisms at all~-- it may be a distributional class rather than a mechanistic kind.
\end{description}
