...
Projectibility isn't metaphysically guaranteed. A proposed grouping can fail to project if its characterisation doesn't match the maintaining mechanisms~-- as aspect-abstract fails. A proposed grouping can fail to project if the mechanisms are too weak or too variable~-- as nonce formations fail. In either case, the grouping isn't a category in our sense; it's a label without the causal structure to back it up. But where the mechanisms are real and consistent, projectibility follows~-- and the grouping earns the name.
