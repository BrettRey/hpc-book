...
Their corpus study of Polish aspect reveals a usage landscape strikingly different from the textbook picture. The key findings:

\begin{enumerate}
    \item \textbf{Lexical bias is the norm.} About 90\% of Polish verbs strongly prefer one aspect—greater than 90\% of their tokens appear in the preferred form. Only 11\% of aspectual pairs are genuinely equiprobable. The textbook suggestion that speakers \enquote{choose} aspect based on how they view the event applies to a small minority of cases.
    
    \item \textbf{Tense carries most of the signal.} A simple model using just the superlemma (verb identity) and three-way tense distinction (past, present, future) achieves F1 scores of 0.95 for imperfective and 0.90 for perfective. Aspect is largely predictable from tense, not from semantic viewpoint.
    
    \item \textbf{Context cues appear where needed.} Temporal adverbs and other contextual markers show up reliably only with the 11\% of verbs that lack lexical bias. The system is informationally efficient: redundant cues don't clutter unambiguous cases.
    
    \item \textbf{The \enquote{choice} is already made.} For most verbs, learning the verb means learning its aspect. The category isn't applied at the point of utterance; it's baked in at lexical acquisition.
\end{enumerate}

This reframes what aspect \emph{is}. It's not a binary semantic opposition applied at the moment of speaking. It's an emergent pattern reflecting the distributional signatures of how verbs are learned and used—a property cluster maintained by mechanisms of acquisition and entrenchment, not a category defined by invariant meaning.
