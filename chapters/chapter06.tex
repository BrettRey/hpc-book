...
This transfer is what mechanistic kindhood predicts. Filler-gap processing~-- tracking a displaced constituent and resolving it to a gap position~-- is maintained by parsing operations that apply across surface variation. The mechanism is exercised every time a speaker produces or comprehends a relative clause, an open interrogative, or a cleft. Each construction reinforces the same parsing routine; the routine in turn makes each construction processable. The stabilisation is reciprocal.

A case study deepens the point. \citet{hankamer1973whose} claimed that English lacks independent relative \mention{whose}~-- forms like \mention{a woman whose was straight} where the possessum is elided. Their constructed examples are indeed ungrammatical: \mention{*The guy whose you saw banging at the window...} fails. For forty years, the construction was treated as a syntactic gap.

But the gap may be illusory. \citet{reynolds2025whose} documents attestations spanning seven centuries, from Middle English manuscripts to contemporary academic prose: \mention{a friend of whose had told us of the accident}; \mention{those whose mental faculties are intact versus those whose are not}. The construction is rare~-- perhaps once in millions of words~-- but attested in edited sources across registers. What Hankamer and Postal diagnosed as syntactic prohibition may instead be pragmatic licensing: the construction requires specific information-structural conditions (contrastive parallelism, deictic anchoring, structural integration) that ensure the elided possessum remains recoverable.\footnote{The grammaticality judgments remain disputed. \textcite{reynolds2025whose} is unpublished, and native-speaker intuitions vary. The epistemic status is accordingly tentative: suggestive evidence that the mechanism is there, not conclusive proof.}

If this analysis is correct, independent relative \mention{whose} is a filler-gap construction~-- \mention{whose} fills a gap in subject or complement position~-- and it's maintained by the same parsing mechanism that Boguraev et al. showed transfers across construction types. The mechanism projects. What looked like a syntactic gap was actually the mechanism operating under unusually stringent pragmatic conditions.

Cross-linguistic evidence supports this interpretation. German and Japanese~-- despite radically different syntactic structures~-- show identical information-structural constraints on independent possessive relatives. The constraints are not language-specific accidents; they're consequences of how the parsing mechanism interacts with discourse accessibility. The mechanism is universal; its licensing conditions are pragmatic.

The contrast with NPIs is now sharper. NPI licensing is lexically specified: each item has its own distribution, and models learn them item by item. Filler-gap is mechanistically unified: a single parsing operation applies across constructions and languages, producing the transfer that Boguraev et al. observed. The operational criterion for an HPC kind~-- cross-item generalisation under intervention or learning transfer~-- is met by filler-gap, not by NPI. Projectibility tracks the difference.

This is the pattern Chapter~\ref{ch:stabilisers} developed in detail for quotatives: a clustering maintained by mechanisms, with causal depth (parsing operations grounded in memory and processing constraints) and reciprocal stabilisation (constructions maintain the mechanism that maintains them). The filler-gap case shows the same structure in syntactic processing.
...
The cross-linguistic term \term{aspect} functions as a comparative concept; within Polish, it names a historically inherited pattern whose contemporary projectibility is anchored in lexeme-specific cue structures rather than the textbook semantic essence. Labels aren't definitions, and labels aren't mechanisms. Calling this opposition \term{aspect} risks reifying a historically entrenched label whose textbook semantics no longer track the mechanisms that actually maintain usage. It is best treated as the name of an emergent clustering, not the explanatory unit that makes it project. But here's a constraint: genealogical continuity alone doesn't justify continued use of a label. The label is appropriate only if it still picks out a stable emergent pattern at some explanatory level. An island that has fully eroded, with no coral keeping it together, is no longer an island~-- and deserves no name.


\section{Generalising the lesson}
\label{sec:6:generalising-lesson}

Polish aspect illustrates a general lesson of the maintenance view. Polish has a grammatical opposition traditionally called aspect; what these results challenge is the assumption that the textbook characterisation of that opposition is the mechanism that makes it projectible. Aspect has been defined for centuries, but those definitions fail to predict usage. By contrast, mechanism-based accounts succeed where definitions fail: the same system becomes predictable once you attend to the distributional structure~-- lexical bias, tense context, morphological cues~-- rather than the semantic essence.

This separation reveals that the category and the mechanism need not coincide. \term{Aspect} is a useful label picking out a real pattern, but the psychological reality of that pattern may be a network of cue--outcome associations, not a represented binary. The mechanism lives below the category; the projectibility comes from the mechanism. When a proposed characterisation fails to project, that's diagnostic: it tells you the characterisation doesn't track the mechanisms. This is HPC without category reification: clusters maintained by mechanisms, labels applied by analysts.
...
The answer is that the framework offers something beyond the mechanisms themselves: an account of what the mechanisms are \emph{for}. Most mechanistic research is descriptive in a specific sense~-- it shows how patterns emerge, stabilise, and change. What it often lacks is an explicit ontology. Are the categories that emerge from these mechanisms \emph{real}, or are they convenient fictions? Are they features of speakers' minds, of the speech community, or merely of the analyst's framework? The maintenance view provides an answer: categories are real to the extent that they're maintained by consistent mechanisms. The mechanisms don't just produce the patterns; they \emph{constitute} the categories as natural kinds.

This matters for several reasons.

First, it legitimises mechanistic work as theory, not just description. A sceptic might object that studying entrenchment or alignment is interesting but atheoretical~-- mere data-gathering that falls short of explanation. The maintenance view says: no. The mechanisms \emph{are} the theory. Understanding how categories are maintained is understanding what makes them real. This is analogous to recognising that natural selection isn't ``just'' a description of breeding patterns but the actual explanation of adaptation.
