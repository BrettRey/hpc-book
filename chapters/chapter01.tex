...
Weak and generic uses of \mention{the} (\mention{go to the hospital}, \mention{the tiger is endangered}) resist the standard definition of definiteness~-- that the referent is identifiable to the hearer. Part~III returns to definiteness; for now, note only that grammars describe these uses without explaining how they fit the category.

Reciprocals like \mention{each other} sit at the boundary between pronouns and compound determinatives; their morphology pulls one way, their distribution and meaning pull another.\footnote{Morphologically, reciprocals pattern with \mention{somebody} and \mention{anybody}: fused determiner--head structures (single words that do the work of both a determiner and a noun). Syntactically and semantically, they behave like pronouns. \textit{CGEL} classifies them as pronouns, but the fit is imperfect in predictable ways~-- mechanism competition rather than analytic failure \citep{reynolds2025reciprocals}.}

Or consider a coerced construction: \mention{I had to think myself through that argument}~-- attested, interpretable, built by analogy to \mention{talk oneself through} and \mention{work oneself through}. But \mention{think} is intransitive; you think about a problem, you don't think it. Here the construction has coerced an intransitive verb into a frame that treats it as capable of moving an entity along a path. Speakers will disagree about whether the verb belongs there at all.
