...
Weak and generic uses of \ixl{the} (\mention{go to the hospital}, \mention{the tiger is endangered}) resist the standard definition of definiteness.\ixsq{definiteness} Part~III returns to definiteness; for now, note only that grammars describe these uses without explaining how they fit the category.
...
And at the social end, \ixs{register} and \ixs{honorific} systems impose boundaries that are real in practice yet gradient in uptake: \ixl{gonna} can be ordinary in casual speech and jarring in academic prose; in honorific systems the choice between plain and polite forms carries systematic inferences.
...
The second response says: categories aren't defined by conditions at all. They are \term{prototype}s\ixsq{prototype}~-- clusters of typical features, with central members and peripheral members and fuzzy boundaries all the way down. A robin is a better example of \textsc{bird} than a penguin is, but both are birds. \ixlq{run}\mention{Run} is a better example of \textsc{verb} than \ixl{beware} is, but both are verbs. Stop expecting sharp edges. \ixs{gradience} is the nature of the beast.
...
None of this means essentialism is useless. For most words, most of the time, the category labels work. \ixlq{dog}\mention{Dog} is a noun; \ixl{run} is a verb; \ixl{quickly} is an adverb. The labels support generalization. They organize grammars. They enable pedagogy. If every word were like \ixl{otherwise}, linguistic analysis would be impossible.
