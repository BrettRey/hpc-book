...
Prototype theory and feature-bundle approaches were right that classical essentialism failed. They were right that categories have gradient structure, fuzzy boundaries, better and worse members. They didn't go far enough: documenting the gradience is not the same as understanding it.
...
Essentialism offers a clear ontology~-- categories are discrete, membership is binary, definitions can in principle be found~-- and this clarity has built the infrastructure of modern linguistics. Grammars, parsers, textbooks, annotation schemes: all of it rests on the assumption that \mention{fun} is either a noun or an adjective, that \mention{otherwise} belongs somewhere. The assumption is false, but it's load-bearing. Take it away and the architecture sags.

Prototype theory offers an accurate phenomenology~-- categories are gradient, boundaries are fuzzy, typicality is real~-- and this accuracy has licensed a generation of work on the messiness that essentialism couldn't see. But accuracy is not explanation. To say that \mention{cattle} is a non-prototypical noun is to redescribe the problem, not to solve it. The gradient structure is stable. Prototype theory records the stability. It doesn't say what produces it.

We are left oscillating. In one mode, we write as if categories were crystalline: discrete, bounded, the kind of thing that could appear in a rule. In another mode, we acknowledge the gradience, note the exceptions, and quietly set them aside. The two modes don't communicate. The crystalline picture is useful but false. The gradient picture is true but inert.
