...
On the HPC view, \mention{cattle} has just the kind of profile you'd expect for an item partially but not fully subject to the mechanisms that maintain nounhood. It's entrenched in noun-phrase constructions: \mention{the cattle}, \mention{these cattle}, \mention{many cattle}. That entrenchment maintains its nominal properties. But it lacks a singular form, which means it's not subject to the full paradigmatic pressure that shapes regular nouns. The mechanisms that would give it a singular~-- analogy to \mention{dog/dogs}, \mention{cat/cats}~-- have never taken hold, perhaps because the referent (a mass of animals, not an individual) doesn't invite singularization in ordinary usage. The clustering is partial because the mechanism application is partial. And it's stable because nothing in the current environment is pushing it to change: the entrenchment that maintains its nominal properties is strong enough to keep it in the category, while the absence of singularization pressure keeps it from acquiring a singular.
