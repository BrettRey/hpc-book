...
\begin{exe}
    \ex \label{ex:12:dog-it} \textit{The dog wagged its tail.}
    \ex \label{ex:12:dog-who} \textit{Who's a good boy? Yes, you are!}
\end{exe}

In (\ref{ex:12:dog-it}), the dog takes the non-personal pronoun \mention{it}. In (\ref{ex:12:dog-who}), addressed directly by its owner, the same animal takes the personal pronoun \mention{who} and receives \mention{you}. Nothing about the dog has changed. What changed is how the speaker conceptualized the referent.
...
\begin{exe}
    \ex \label{ex:12:doctor-who} \textit{The doctor who I saw was helpful.}
    \ex \label{ex:12:doctor-which} \ungram{\textit{The doctor which I saw was helpful.}}
    \ex \label{ex:12:book-which} \textit{The book which I read was helpful.}
    \ex \label{ex:12:book-who} \ungram{\textit{The book who I read was helpful.}}
\end{exe}

The constraint is robust in standard written English. \mention{Who} for persons, \mention{which} for non-persons. The violations in (\ref{ex:12:doctor-which}) and (\ref{ex:12:book-who}) aren't merely odd~-- they're ungrammatical, comparable to agreement errors or subcategorization violations. This isn't pragmatic markedness; it's grammar.

The same split appears in interrogatives:

\begin{exe}
    \ex \label{ex:12:int-who} \textit{Who is coming to the party?} \hfill [asking about persons]
    \ex \label{ex:12:int-what} \textit{What is on the table?} \hfill [asking about non-persons]
    \ex \label{ex:12:int-who-bad} \ungram{\textit{Who is on the table?}} \hfill [under construal: asking about objects]
    \ex \label{ex:12:int-what-bad} \ungram{\textit{What is coming to the party?}} \hfill [under construal: asking about persons]
\end{exe}

Examples (\ref{ex:12:int-who-bad}) and (\ref{ex:12:int-what-bad}) are grammatical only under shifted construals~-- if the speaker expects a person on the table or a non-person attending the party. The choice of interrogative reveals the speaker's conceptualization of the potential referent.
...
\addlinespace
\mention{this}, \mention{that} & & & \mention{here} \\
\addlinespace
...
Similarly, relative \mention{whose} is gender-neutral (\mention{the person/book whose story is well known}), but interrogative \mention{whose} is personal (\mention{Whose coat is this?} presupposes a person). The split reflects how these items function: relative \mention{whose} picks up gender from its antecedent; interrogative \mention{whose} presupposes personhood independently.
...
\begin{exe}
    \ex \label{ex:12:metonymy} \textit{The French fries is waiting. She's upset.} \hfill \autocite{wu2025}
\end{exe}
...
\begin{exe}
    \ex \label{ex:12:spanish}
    \ea[*]{\gll \textit{Las patatas fritas} espera. \textit{Él} está enfadado.\\
    ...
    \ex[]{\gll \textit{Las patatas fritas} esperan. \textit{El señor} está enfadado.\\
...
Spanish requires grammatical agreement with the antecedent (\textit{patatas}, feminine plural), not semantic agreement with the metonymic designatum. The speaker needs a rephrasing with a full NP like \mention{\textit{el señor}} (`the gentleman') to achieve the intended reference. English permits categorical shifts based on conceptualization because it lacks NP-internal gender agreement: the designatum-driven pathway is freer. This is the signature of a designatum-driven system.


%--- --- --- --- --- --- --- --- --- --- --- --- --- --- --- --- ---
\section{Beyond the \mentionhead{he}/\mentionhead{she}/\mentionhead{it} triad}
...
What unifies pro-forms is their \term{substitution function}: they designate by standing for something else. A pronoun stands for an NP; a pro-adverb stands for an adverb phrase; a wh-word stands for a constituent in a question or relative clause. This isn't a morphosyntactic category (there's no frame that selects for ``any pro-form''), but a semantic one. The gender constraint doesn't respect lexical-category boundaries: whatever property licenses \mention{who} also licenses \mention{somebody} and blocks \mention{something}.

Recall the skink in the epigraph, whose \mention{one's} is epicene personal despite referring to a lizard. The speaker has construed the skink as capable of learning~-- an attribution of intentionality that licenses personal gender. The constraint applies across the entire substitution class.
...
\begin{exe}
    \ex \label{ex:12:somebody} \textit{Somebody left their coat.} \hfill [personal]
    \ex \label{ex:12:somebody-bad} \ungram{\textit{Somebody fell off the table.}} \hfill [under construal: referring to an object]
\end{exe}

Example (\ref{ex:12:somebody-bad}) is ungrammatical unless \mention{somebody} refers to a person who fell. These are epicene~-- they don't encode sex~-- but they do encode the personal/non-personal distinction.

The compound determinatives with \mention{-thing} and \mention{-where} are non-personal:

\begin{exe}
    \ex \label{ex:12:something} \textit{Something is on the table.} \hfill [non-personal]
    \ex \label{ex:12:something-bad} \ungram{\textit{Everything enjoyed themselves.}} \hfill [under construal: personal referents]
\end{exe}
...
\begin{exe}
    \ex \label{ex:12:room-where} \textit{the room where the painting was done} \hfill [room as location]
    \ex \label{ex:12:room-which} \textit{the room which was painted} \hfill [room as patient]
\end{exe}

\begin{exe}
    \ex \label{ex:12:2010-when} \textit{2010, when I left} \hfill [as time]
    \ex \label{ex:12:2010-which} \textit{2010, which has 365 days} \hfill [as entity]
\end{exe}
...
\begin{exe}
    \ex \label{ex:12:dog-chain-a} \textit{That's the dog who attacked his owner.} \hfill [personal chain]
    \ex \label{ex:12:dog-chain-b} \textit{That's the dog which attacked its owner.} \hfill [non-personal chain]
    \ex[?]{\label{ex:12:dog-chain-c} \textit{That's the dog which attacked his owner.} \hfill [mixed]}
    \ex[?]{\label{ex:12:dog-chain-d} \textit{That's the dog who attacked its owner.} \hfill [mixed]}
\end{exe}
...
On the morphosyntactic side sits the \term{pro-form inventory}: the gender-sensitive lexical items and their distributional constraints~-- \mention{who}/\mention{which} in relative clauses, \mention{-body}/\mention{-thing} in compound determinatives, designated slots for personal and non-personal reference.
...
The personhood distinction appears remarkably widespread. \textcite{haspelmath1997} notes that ``the distinction between human and non-human referents is made practically everywhere (`who' vs.\ `what', `somebody' vs.\ `something'), even in languages where humanness isn't very prominent elsewhere in the grammar'' (34--35). Languages with rich grammatical gender (Spanish \mention{quién}/\mention{qué}, German \mention{wer}/\mention{was}, French \mention{qui}/\mention{quoi}) still make the personhood split in their interrogative and indefinite systems. The personhood partition may be cognitively more basic than the particular morphosyntactic systems that realize it.
...
\begin{exe}
    \ex \label{ex:12:cat-it} \textit{The cat licked its paw.} \hfill [non-personal default]
    \ex \label{ex:12:cat-she} \textit{Poor Whiskers! She's not feeling well.} \hfill [personified]
\end{exe}
...
\begin{exe}
    \ex \label{ex:12:team-its} \textit{The team has scored its first goal.} \hfill [singular, non-personal]
    \ex \label{ex:12:team-their} \textit{The team have scored their first goal.} \hfill [plural, personal]
\end{exe}

The variation tracks construal: team-as-entity versus team-as-members. But singular personal construal is blocked:

\begin{exe}
    \ex \label{ex:12:team-themself} \ungram{\textit{The team has scored on themself.}}
\end{exe}

Collectives can be singular non-persons or plural persons, but not singular persons. This reveals the structure of the personhood cluster: singular personhood requires individuation that collective nouns don't provide.
...
\begin{exe}
    \ex \label{ex:12:baby-it} \textit{The baby was crying because it was hungry.}
\end{exe}
...
\begin{exe}
    \ex[?]{\label{ex:12:which-he} \textit{That's the dog which attacked his owner.}}
\end{exe}
...
\begin{exe}
    \ex[??]{\label{ex:12:what-he} \textit{What attacked his owner was this dog.}}
    \ex[*]{\label{ex:12:something-he} \textit{Something attacked his owner.} \hfill [with coreference]}
\end{exe}
