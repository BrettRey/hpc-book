...
I'll argue that English gender, like countability (Chapter~\ref{ch:countability}) and definiteness (Chapter~\ref{ch:definiteness-and-deitality}), exhibits the HPC architecture: two clusters~-- one semantic (personhood), one lexico-grammatical (the pro-form inventory)~-- coupled by designatum-driven inference and maintained by overlapping mechanisms. Section~\ref{sec:8:coupling} introduced a coupling continuum~-- hard at the phoneme level, loose at the grammatical level, composite in constructions. Gender is where coupling tightness and domain breadth come apart. The personhood--pro-form link is transparent and actively maintained~-- closer to countability's tight coupling than to definiteness's productive slippage. But the domain is a few dozen pro-forms, not all NPs. The coupling is tight; the scope is narrow.

Chapter~\ref{ch:projectibility} argued that the interpretant is the habit a sign generates, and that the same sign can generate different habits under different conditions. Gender makes this vivid. The pro-form inventory (form) and the personhood construal (object) are coupled by a habit: hear \mention{who} and you expect a person; construe something as personal and you use \mention{who}. But the habit isn't fixed by the referent's properties. The dog in both examples above is the same animal; what shifts is the interpretant~-- the speaker's construal of personhood~-- and with it, the form. This is Peirce's point about conditional habits: the would-be depends on circumstances, not just on what the sign is \enquote{about}. Same referent, different construals, different habits, different forms. Violations aren't just semantic infelicities; they're grammatical errors. The system has teeth~-- though some are sharper than others, and a few are mostly for display. Some constraints are categorical in standard written English (the \mention{who}/\mention{which} split; core \mention{he}/\mention{she}/\mention{it} uses); others show the graded edge behaviour typical of HPC kinds (compound determinatives under shifted construals; chain-coherence effects). The interest lies in explaining both the categorical core and the gradient periphery. Pro-form gender is a narrow basin~-- deep within its domain but small in scope.
...
morphosyntactic category (there's no frame that selects for \enquote{any pro-form}),
...
Knowing the designatum's construal predicts pro-form selection. If you know the speaker is construing a pet as personal, you can predict \mention{who} over \mention{which}, \mention{he}/\mention{she} over \mention{it}. The predictions are bidirectional: from form to construal, from construal to form.
...
Four case studies, four coupling profiles. Countability is maintained by a dense web of morphosyntactic and semantic mechanisms; definiteness splits into two partially independent clusters; lexical categories range from skeleton-thick to wastebasket-empty; and pro-form gender couples personhood to morphosyntax through a tight but constructionally sensitive system. Chapter~\ref{ch:the-category-zipper} maps these profiles onto a single gradient~-- from phonemes at the hard-coupled end to constructions at the conventional end~-- and asks what keeps the teeth engaged at each tier.
