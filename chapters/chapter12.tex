...
This chapter shows why that restriction is too narrow. The same personhood-based logic that governs \mention{he}/\mention{she}/\mention{it} also governs \mention{who}/\mention{which}, \mention{somebody}/\mention{something}, and \mention{when}/\mention{where}. The system extends across the entire class of items that function semantically as pro-forms.\footnote{Pro-forms are semantically defined~-- they substitute for larger constituents~-- but morphosyntactically heterogeneous: pronouns, determinatives, pro-adverbs, and wh-words belong to different lexical categories. What unifies them here isn't syntactic homogeneity but participation in a shared gender system. Whether that shared participation is sufficient to make `pro-form' a genuine category is a question for Chapter~\ref{ch:lexical-categories}; for present purposes, the claim is only that the personhood constraint applies wherever the substitution function does.}
...
\term{Indexicality}~-- the association of linguistic forms with social meaning~-- is unusually prominent in gender. When a speaker calls a ship \mention{she} or addresses a pet as \mention{who}, they're not just selecting a pro-form; they're signalling an attitude. The choice indexes affection, anthropomorphism, or tradition. This social freight makes gender-marked forms resistant to change: speakers don't want to lose the expressive resource. Indexicality explains why personification persists as a productive pattern even though the grammar doesn't require it.
...
These three~-- cognitive grounding, indexicality, error-and-repair~-- account for the system's distinctive stability. Standard mechanisms (acquisition, entrenchment, transmission) also apply, but they don't explain why gender is more stable than, say, the count/non-count distinction for \mention{data}. What explains the stability is the convergence: personhood is cognitively basic, socially indexed, and repaired when violated.
