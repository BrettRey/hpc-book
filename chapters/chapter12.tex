...
\subsection{Interference from number}
\label{sec:12:number}
...
Dialect and register norms amplify the variation. British English more readily accepts plural verb agreement with singular collective heads; American English prefers singular (\textit{cf.}\ Associated Press vs.\ BBC style guides). What's interesting for the HPC analysis is that the combination of singular verb with plural personal anaphora (\mention{The team has changed their strategy}) is widely tolerated~-- the two agreement systems (verbal, pronominal) can diverge when pulled by different pressures. The mechanism that stabilizes the pattern is register policing: formal written English converges on singular-singular or plural-plural; informal speech permits the mix.

\subsection{Competition from other pressures}
...
\z
This is not a denial of personhood. It reflects competition between personhood and other communicative pressures~-- particularly sex-marking and specificity. When the infant's sex is unknown or irrelevant, \mention{it} allows the speaker to avoid the sex-based \mention{he}/\mention{she} alternation. When the discourse is generic (\mention{A baby needs its mother}), the non-referring construal aligns with non-personal anaphora. And in some contexts, as \textcite{mcconnell-ginet2014} notes, the choice of \mention{it} can carry ideological weight~-- positioning the infant as not-yet-fully-a-person, though this is not the default reading.
...
\subsection{Lexeme-specific anchoring}
\label{sec:12:anchoring}

Not all non-personal pro-forms behave identically when chains continue. Relative \mention{which}, tethered to an antecedent NP, can allow later personal anaphora:

\ea[]{\label{ex:12:which-chain}\mention{The dog which attacked the farmer ran away. He was never caught.} \hfill [upgraded to personal]}
\z
The antecedent (\mention{the dog}) carries enough information~-- animacy, individuality~-- that hearers can reconstruct a personal designatum and accept the shift.

But \mention{what} in free relatives and \mention{something} are systematically underspecified. They lack a full NP antecedent, so there's less material for hearers to anchor a personal construal:

\ea[{}]{\label{ex:12:what-chain}\mmark{\mention{What attacked the farmer ran away. He was never caught.}}}
\z
\ea[{*}]{\label{ex:12:something-chain}\mention{Something attacked the farmer. He was never caught.} \hfill [with coreference]}
\z
The contrast is not simple gradience within the non-personal domain; it's a difference in discourse anchoring. Relative \mention{which} points to an existing NP that can be ``upgraded''; \mention{what} and \mention{something} provide no such anchor, so the chain resists promotion to personal. The mechanism that stabilizes the pattern is chain coherence: when anchor is weak, shift is blocked.
