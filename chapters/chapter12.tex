...
I'll argue that English gender, like countability (Chapter~\ref{ch:countability}) and definiteness (Chapter~\ref{ch:definiteness-and-deitality}), exhibits the HPC architecture: two clusters~-- one semantic (personhood), one lexico-grammatical (the pro-form inventory)~-- coupled by designatum-driven inference and maintained by overlapping mechanisms. In the triadic terms of Chapter~\ref{ch:the-category-zipper}:

\begin{itemize}
    \item \textbf{Form}: the pro-form inventory~-- \mention{who}/\mention{which}, \mention{he}/\mention{she}/\mention{it}, \mention{somebody}/\mention{something}.
    \item \textbf{Object}: the personhood construal~-- whether the referent is personal, non-personal, or somewhere in between.
    \item \textbf{Interpretant}: the inferential habit~-- expectations about referent type, animacy, agentivity, and chain coherence across discourse.
\end{itemize}

What couples form and object is the interpretant: when you hear \mention{who}, you expect a person; when you construe something as personal, you use \mention{who}. The dog in both examples is the same animal; what shifts is the interpretant~-- and with it, the form. Violations aren't just semantic infelicities; they're grammatical errors. The system has teeth~-- though some are sharper than others, and a few are mostly for display. Some constraints are categorical in standard written English (the \mention{who}/\mention{which} split; core \mention{he}/\mention{she}/\mention{it} uses); others show the graded edge behaviour typical of HPC kinds (compound determinatives under shifted construals; chain-coherence effects). The interest lies in explaining both the categorical core and the gradient periphery. Pro-form gender is a narrow basin~-- deep within its domain but small in scope.
