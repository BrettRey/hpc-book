...
This chapter answers the question that the previous three have been building toward: what keeps grammatical categories stable? The answer is mechanisms~-- multiple, interacting, operating at different timescales. Rather than enumerate mechanisms abstractly, the chapter develops two case studies in depth: the emergence of new quotative markers and the licensing of independent relative \mention{whose}. Both show the same structure: a cluster of properties maintained by a braid of stabilisers, with causal depth, mereological structure, and reciprocal maintenance at every level. A deeper point emerges: the distinction between ``category'' and ``mechanism'' is perspectival, not ontological~-- mechanisms are themselves categories, maintained by further mechanisms, all the way down. The chapter closes with operational tests for mechanism claims, a discussion of what the framework commits us to, and a reminder that the maintenance view refactors existing research rather than replacing it.
...
When immunologists answer the analogous question for cell types, they don't point to a single cause. They narrate a multi-level story. The same style of explanation applies to grammatical categories.
...
This reframes the ``dual category'' problem. The question isn't which category \mention{fast} belongs to. The question is which mechanisms are activated by which inputs. In the adjective basin, the form takes comparison (\mention{faster}, \mention{fastest}) and attributive position; in the adverb basin, it takes post-verbal position and modifies verbs. Both are available; context determines activation. (A caution: ``activation state'' is a useful way of describing variability under stabilising pressures, not a new single-factor key. It's a description of the dynamics, not a replacement essence.)
...
A clarification about two kinds of variation. Some apparent boundary cases are \emph{within-speaker} shifts~-- the same person using \mention{fast} adjectivally in one context and adverbially in another. These are activation-state differences: one grammar, multiple accessible patterns. Others are \emph{between-speaker} differences~-- not everyone has entrenched the same patterns to the same depth. Speakers with different input histories, different literacy experiences, different interactional ecologies end up with different category boundaries. Both matter. The within-speaker variation shows that categories are context-sensitive; the between-speaker variation shows that they are experience-dependent. Neither is noise. Both are part of the data that a theory of category maintenance must explain.
...
This is the processing → stability bridge. Online processing pressures~-- prediction, retrieval ease, reduced production cost under repetition~-- preferentially reinforce some cluster features over others. The features that are easy to predict, easy to produce, and easy to retrieve are the ones that survive and spread. That selective reinforcement keeps the category coherent across time and speakers. Processing isn't just something that happens to categories; it's one of the mechanisms that maintains them.


\subsection{How deep do mechanisms go?}

The stabilisers just described~-- processing economy, expressive fit, social indexing, transmission dynamics~-- might look like endpoint explanations. But mechanisms have mechanisms. Each can be decomposed causally (what underlies it?) and mereologically (what are its parts?). A quick illustration.

Take the robust finding that young women lead quotative innovation \autocite{labov2001,tagliamontedarcy2004}. This isn't a brute fact. It bottoms out in social-psychological structure: dense peer networks in adolescence \autocite{milroy1987}, separation-individuation dynamics that make linguistic innovation socially functional \autocite{erikson1968}, prolonged cohort co-location in schooling. \enquote{Young women lead change} is the surface manifestation of a causal cascade~-- distal factors shape network structure, which shapes input frequency, which shapes entrenchment. A caution: the evidence requirements tighten as we descend levels. The sociolinguistic pattern is robust; the network-density explanation is well-supported; the developmental-psychology account is plausible but less directly tested. Readers who find the deeper levels overreaching can stop at network density without losing the main argument.

The deeper point: if categories are projectible clusters stabilised by mechanisms, and if the mechanisms themselves are projectible clusters stabilised by further mechanisms, then mechanisms \emph{are} categories. \enquote{Processing economy} bundles properties (reduced forms, high frequency, resistance to analogy) that cluster because of underlying neural constraints. \enquote{Social indexing} bundles properties (youth association, in-group marking) maintained by identity construction. The mechanisms we invoke are themselves HPC kinds. And the stabilisation is reciprocal: quotatives don't just \emph{use} processing economy; they \emph{maintain} it. Every token reinforces the facilitation that makes high-frequency forms easier. The category and the mechanism co-construct each other~-- precisely the homeostatic structure that gives HPC kinds their stability.

A contrast case sharpens the point. Consider the traditional category \term{particle}~-- a wastebasket label for uninflected words that don't fit elsewhere. Does this cluster survive a maintenance audit? The properties don't co-occur reliably; there's no common acquisition pathway; predictions trained on one subtype fail to transfer to another. The label exists; the mechanism-maintained kind doesn't. This is what failure of reciprocal maintenance looks like. Chapter~\ref{ch:failure-modes} develops such failure modes systematically.
...
This is what \enquote{same category across languages} means in the maintenance view. Not shared essence. Not definitional identity. Convergent stabilisation. The convergence is not mysterious: if processing economy and expressive fit are universal~-- all humans favour light forms under production pressure, all humans need vague-reference quotatives for storytelling~-- then similar sociolinguistic dynamics will produce similar outcomes wherever they operate. But \enquote{similar outcomes} is not \enquote{identical mechanisms}. The framework predicts family resemblance in the stabilising story, not clones.
...
The \mention{whose} case illustrates what the chapter has been arguing: a shared cognitive mechanism (filler-gap processing) operates under a specific constraint profile (information-structural licensing), and that profile recurs across typologically unrelated languages because the constraints track universal processing pressures, not language-specific syntax. The cross-linguistic similarity is not coincidence; it's convergent maintenance~-- the same mechanisms, operating under the same pressures, producing the same constraints on independent possessive relatives. And the maintenance is reciprocal: filler-gap constructions exercise the processing mechanism that makes them possible, while the mechanism makes the constructions processable in turn. This is what ``labels aren't mechanisms'' means in practice: the label ``ungrammatical'' predicted nothing; the mechanism story predicts exactly when the construction succeeds and when it fails.
...
At this point the metaphors have to pay rent. The biological analogy is not evidence that language \emph{is} biology; it's a reminder that when definitions fail, mechanism-first inquiry is standard scientific practice. The ``stabiliser'' label is not a discovery; it's a way of organising claims that can be tested. If the tests fail, the metaphor was ornamental. If they succeed, the metaphor was doing inferential work.
