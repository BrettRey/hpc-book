...
These natural experiments don't merely show that countability varies; they confirm that the variation is \emph{mechanism-sensitive}. Remove the singular default, and the cluster realigns. Remove obligatory number, and the cluster disperses or reallocates. Reweight the semantics, and the hierarchy collapses. The perturbations behave exactly as changes to a mechanism-maintained equilibrium should. Table~\ref{tab:09:ablation-summary} summarises these shifts.

\begin{table}[ht]
\centering
\small
\caption{Geometry shifts in countability: Comparison of ablation effects}
\label{tab:09:ablation-summary}
\begin{tabular}{@{}llll@{}}
\toprule
\textbf{Perturbation} & \textbf{Locus of Coupling} & \textbf{Outcome} & \textbf{Example} \\ \midrule
Baseline & Noun stem + Morph & Rigid Cluster & English \\
Collective Default & Suffix (Singulative) & Attractor shift & Welsh \\
Reallocation & Classifier Phrase & Cluster shifts to Cl & Mandarin \\
Weakening & Optional Adjunct & Lower Projectibility & Halkomelem \\
Semantic Reweight & Numeral-Noun Interface & Hierarchy Collapse & Yudja \\ \bottomrule
\end{tabular}
\end{table}

This is the diagnostic signature of an HPC kind. The cross-linguistic evidence confirms that English countability is not a metaphysical given but a real category~-- one maintained by the very mechanisms we have identified.

We can watch this mechanism claim a new word in real time. \mention{Emoji} entered English from Japanese around 2010. Japanese lacks obligatory plural morphology, so the word arrived with an unmarked form~-- neither singular nor plural, just \mention{emoji}. But the referents~-- discrete little pictures, each one distinct~-- screamed individuation. The AP Stylebook, like the LEGO Group's lawyers, insists on the invariant form: \mention{three emoji}, not \mention{emojis}. And like the LEGO Group's lawyers, the AP is losing. Within a decade, \mention{three emojis} had overtaken the prescribed form in edited prose. The invariant plateau eroded; usage avalanched into the count basin, and the word acquired its \mention{-s}.
