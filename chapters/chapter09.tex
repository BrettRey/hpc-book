...
\begin{itemize}
    \item \textit{Boundedness}: The referent has discrete edges~-- a spatial or conceptual boundary that separates it from its environment.
    \item \textit{Atomicity}: The referent is accessible as a unit~-- something that can be singled out, pointed to, tracked.
    \item \textit{Enumerability}: The referent is compatible with exact counting~-- three of them, not just much of it.
    \item \textit{Homogeneity resistance}: Parts of the referent are not the same kind as the whole. Half a cat is not a cat; half of water is still water.
\end{itemize}
...
\begin{itemize}
    \item \textit{Denotation}: Discrete, bounded entities.
    \item \textit{Pluralization}: It inflects for number (\mention{cats}).
    \item \textit{Determinatives}: It takes exact cardinals (\mention{one cat}, \mention{two cats}) and fuzzy count quantifiers (\mention{many}, \mention{fewer}).
    \item \textit{Agreement}: It triggers singular or plural verb agreement matching its morphology.
\end{itemize}

For a prototypical non-count noun like \mention{water}:
\begin{itemize}
    \item \textit{Denotation}: Undifferentiated substance.
    \item \textit{Pluralization}: No plural form (*\mention{waters} is special, meaning `types of' or `bodies of').
    \item \textit{Determinatives}: It rejects cardinals (*\mention{two waters}) and takes mass quantifiers (\mention{much}, \mention{less}).
    \item \textit{Agreement}: Always singular.
\end{itemize}
...
\begin{itemize}
\item \textit{Comprehension direction:} Count morphosyntax $\to$ individuated construal. Hearing \mention{a}, \mention{three}, or \mention{many} with a noun leads the hearer to expect atomic, enumerable referents.
\item \textit{Production direction:} Individuated construal $\to$ count morphosyntax. A speaker who has individuated the referents will reach for \mention{a}, \mention{three}, or \mention{many}.
\end{itemize}
...
\begin{enumerate}
\item \textit{Singular form / \mention{a(n)}} — Requires identifying exactly one atomic unit
\item \textit{Low cardinals (\mention{three}, \mention{five})} — Requires enumerating precise atomic units
\item \textit{\mention{Several}} — Requires multiple discrete units, tolerates approximate quantity (\enquote{more than two, not many})
\item \textit{Distributives (\mention{each}, \mention{every})} — Requires discrete units for distribution
\item \textit{\mention{Many}/\mention{few}} — Requires only relative magnitude assessment, not precise enumeration
\item \textit{High round numerals (\mention{a hundred}, \mention{thousands of})} — Function as approximate measures
\item \textit{Plural agreement} — Requires only non-singular construal
\end{enumerate}
...
\begin{itemize}
\item \term{Bidirectional inference} couples morphosyntax to individuation, making the properties mutually reinforcing through a shared semantic variable.
\item \term{Acquisition} transmits the cluster as a unit: children overgeneralise, treating one count property as evidence for the rest.
\item \term{Entrenchment} preserves high-frequency patterns as chunks, resisting analytic decomposition.
\item \term{Institutional norms} stabilise the community-level distribution.
\item \term{Functional anchoring} bleeds pressure on intermediates, explaining why quasi-count nouns persist rather than regularising.
\end{itemize}
