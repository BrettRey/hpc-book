...
To a lawyer, \mention{LEGO} is an adjective, a modifier that tracks a brand origin. To a six-year-old, a plastic brick is a discrete, bounded, manipulable object. And the human language faculty has a hard-wired mechanism for dealing with discrete, bounded, manipulable objects: it counts them. It adds an \mention{-s}. It makes them plural. When a child asks for \mention{three Legos}, they aren't making a mistake; they are obeying a grammatical imperative deeper than any trademark. The count cluster is claiming the word.

This chapter is about that mechanism. It is about why \mention{three Legos} sounds natural while \mention{three furnitures} sounds wrong, even though both refer to collections of discrete objects. It is about why the Minnesota House of Representatives once paused a legislative session to vote on whether the word \mention{data} is singular or plural. And it is about how the HPC control loop we built in Part II handles one of the messiest, most debated distinctions in English grammar. It also predicts the future. As new entities enter our reality, the mechanism processes them. \mention{Emoji} is the latest test case. Borrowed from Japanese (where plural marking isn't obligatory), it hit English speakers' brains around 2010. Immediately, the mechanism asked: Is this stuff, or is it a thing?
\begin{quote}
    \mention{I sent three emoji.} (Preserving the loan syntax / zero-plural)
    \mention{I sent three emojis.} (Regularizing to the count cluster)
\end{quote}
My money is on \mention{emojis}. The individuation is too strong. The little pictures are too discrete. The basin of attraction is waiting.

The count/mass distinction isn't a static binary. It's a dynamic negotiation. And as we'll see in the next chapter, it's just one example of how the maintenance loop builds the structure of our world. If the count/mass distinction were a simple binary switch, we'd expect words to flip cleanly from one side to the other. What we actually see is \mention{cattle}, \mention{police}, and \mention{vermin} sitting stably in the middle for five centuries. We see \mention{data} tearing itself apart in real time. We see \mention{emoji} entering the language and immediately triggering the same choice: one emoji, two emojis?

Countability, I'll argue, isn't a definition. It's a homeostatic property cluster. The properties cluster because they are inferentially coupled to a common semantic variable~-- individuation~-- and they dissociate in a predictable order when that variable weakens.
...
This is why children say \mention{I saw three sheeps}. The child perceives discrete animals. The grammar provides a slot for discrete animals (the regular plural). The child fills the slot, ignoring the lexical exception. The semantic variable (Individuation) is driving the morphosyntax. The error proves the mechanism works: the child is running the algorithm perfectly, just on the wrong input.
...
On March 11, 2015, the Minnesota House of Representatives Judiciary Finance and Policy Committee halted its work on a license-plate reader bill. Representative John Lesch had spotted a grammatical error. The bill text read \mention{the data are private}. Lesch, a stickler, insisted that \mention{data} is singular. \mention{Unless we want to start saying `The water in this building are safe',} he argued, \mention{we should be consistent.}

The committee debated. They voted. The motion passed unanimously. Representative Lesch pumped his fist in the air. The law was amended: \mention{data is}.
