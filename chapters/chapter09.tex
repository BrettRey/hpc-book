...
There's a cognitive dimension to why high-frequency patterns resist change. Following \textcite{bybee2010}, we can understand entrenchment in terms of \term{chunking}: high-frequency sequences get stored as units and accessed as wholes, bypassing compositional assembly.
...
For readers new to ABM, the assumptions are deliberately simple. Each agent stores one latent individuation score per noun. English constructions are modelled as thresholds on that score: \mention{a(n)} and low cardinals are tight thresholds, \mention{many} and agreement are looser thresholds. On each interaction, a speaker chooses a communicative intent, produces a packaging choice, and a hearer updates its score from what it observes. For transmission, a small share of adults is periodically replaced by new learners with weak priors who bootstrap from finite tutor exposure. The current chapter plots use 100 agents, including 5 institutional agents. This is a computational shortcut for maintenance dynamics, not a full developmental model of child acquisition bottlenecks. When scaling $N$, we scale interaction budget with it; otherwise per-agent exposure collapses and the apparent dynamics become a sparse-update artifact.

The model operationalizes a comparative concept (individuation) and maps it to English packaging frames. So the claim is conditional and language-scoped: in this English-targeted toy system, when the singulative anchor (\mention{datum}) erodes, tight properties should weaken earlier than loose ones.
...
If bidirectional inference is a broader mechanism~-- and not only an English pattern~-- languages with different morphological resources should show analogous signatures.
...
\textcite[532]{grimm2018} shows that Welsh base-form collectives behave very similarly to English quasi-count nouns: they accept loose properties (quantifiers like \mention{llawer} `much/many') but reject tight properties (numerals) without singulative marking. The \mention{cattle} profile isn't a quirk of English irregulars; it's a plausible attractor state for aggregate nouns when the pressure to act as a singular is removed. English \mention{cattle} can be read as a comparable pattern in a number-marking language.
