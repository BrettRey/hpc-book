...
These properties cluster. When a referent is bounded, it's usually atomic; when it's atomic, it's usually enumerable; when it's enumerable, it usually resists homogeneous subdivision. The clustering isn't accidental. But it isn't absolute. A \mention{ball bearing} is maximally individuated: bounded, atomic, enumerable. But a \mention{cloud} is bounded while lacking stable atomicity\ixsq{atomicity}. \mention{Cattle} denotes discrete animals, but the noun construes them as an aggregate. The cluster represents a prototype\ixsq{prototype}, not a rigid definition.
...
This is the \term{count cluster\ixsq{count cluster}}: the set of formal properties that normally travel together. When you hear a word has a plural form, you can predict it will take \mention{many} and reject \mention{much}. You can predict it will refer to something conceptualized as an individual.
...
There's a cognitive dimension to why high-frequency patterns resist change. Following \textcite{bybee2010}, we can understand entrenchment\ixsq{entrenchment} in terms of \term{chunking\ixsq{chunking}}: high-frequency sequences get stored as units and accessed as wholes, bypassing compositional assembly.
...
When we say \mention{cattle} clears the loose locks but fails the tight ones, we're saying its top spins stably in the basin\ixsq{basin}~-- but off-centre. It's count-ish. It's in the attractor. But it's not at the core.

\mention{Book} spins at the dead centre, satisfying every precision demand. \mention{Cattle} spins off-centre but stably~-- it's in the count basin\ixsq{basin}, but it can't reach the precision locks at the centre. \mention{Folks}~-- we'll come to this~-- wobbles. Sometimes it clears the \mention{three} lock, sometimes it doesn't. Its position in the basin\ixsq{basin} is unstable.
...
A noun that loses \mention{several} will already have lost \mention{a(n)} and low cardinals. A noun that retains only plural agreement will have lost everything above it. The properties form an implicational scale, and a noun's countability\ixsq{countability} profile is its position on that scale.
...
The instability of \mention{folks} isn't noise. It's structure. The noun is wobbling in the basin\ixsq{basin}, uncertain whether it will stabilize off-centre (like \mention{cattle}), drift outward (toward mass), or regularise toward the centre (developing tight properties). The outcome depends on whether a functional anchor emerges or prescriptive pressure crystallises.
...
For readers new to ABM, the assumptions are deliberately simple. Each agent stores one latent individuation score per noun. English constructions are modelled as thresholds on that score: \mention{a(n)} and low cardinals are tight thresholds, \mention{many} and agreement are looser thresholds. On each interaction, a speaker chooses a communicative intent, produces a packaging choice, and a hearer updates its score from what it observes. For transmission\ixsq{transmission}, a small share of adults is periodically replaced by new learners with weak priors who bootstrap from finite tutor exposure. The current chapter plots use 100 agents, including 5 institutional agents. This is a computational shortcut for maintenance dynamics, not a full developmental model of child acquisition bottlenecks. When scaling $N$, we scale interaction budget with it; otherwise per-agent exposure collapses and the apparent dynamics become a sparse-update artifact.

The model operationalizes a comparative concept\ixsq{comparative concept} (individuation) and maps it to English packaging frames. So the claim is conditional and language-scoped: in this English-targeted toy system, when the singulative\ixsq{singulative} anchor (\mention{datum}) erodes, tight properties should weaken earlier than loose ones.

In representative runs under this parameterization, acceptance for \mention{a(n)} and \mention{three} falls substantially as anchor strength declines, while \mention{many} and agreement remain near ceiling over the same window. The model therefore captures asymmetric fraying rather than immediate category collapse.

\begin{figure}[htpb]
\centering
\includegraphics[width=1\textwidth]{figures/data_drift_experiment.png}
\caption{Simulated \mention{data} drift in an English-specific toy model. Panel A tracks anchor erosion over phases: declining \mention{datum} availability, declining \mention{datum} individuation, and the associated shift in \mention{data} individuation. Panel B shows the corresponding constructional profile. Tight properties (\mention{a(n)}, low cardinals) weaken first because they require reliable singular anchoring; looser properties (\mention{many}, agreement) remain comparatively robust over the same interval.}
...
If bidirectional inference\ixsq{bidirectional inference} is a broader mechanism~-- and not only an English pattern~-- languages with different morphological resources should show analogous signatures.

Welsh and Arabic mark singulatives morphologically. In Welsh, the base form of \mention{adar} (\enquote{birds}) is collective~-- grammatically singular, semantically aggregate. A suffix derives the singulative\ixsq{singulative}: \mention{aderyn} (\enquote{a bird}). \textcite{grimm2018} documents exactly the predicted pattern: bare collectives accept loose quantifiers but resist low numerals and distributives, while singulative\ixsq{singulative}-marked forms accept the full count cluster\ixsq{count cluster}. The parallel to English quasi-count nouns is striking: Welsh collectives occupy the same position in the hierarchy that \mention{cattle} and \mention{police} occupy in English.

Classifier\ixsq{classifier} languages (Mandarin, Japanese) encode individuation differently~-- through classifiers that mediate between numerals and nouns, rather than through inflection on the noun itself. The prediction is that the clustering dynamics should shift to the classifier\ixsq{classifier} system: general classifiers should show tight/loose asymmetries parallel to what English quantifiers show. This remains programmatic~-- the cross-linguistic work hasn't been done to the depth needed~-- but the mechanism should apply wherever languages encode individuation morphosyntactically.
...
There's a puzzle lurking in the data. The hierarchy is continuous~-- properties shade from tight to loose~-- but judgments are often sharp. \mention{Three cattle} isn't \enquote{slightly bad} for most speakers; it's simply ungrammatical\ixsq{ungrammatical}. \mention{Many cattle} isn't \enquote{slightly good}; it's fully acceptable. The boundary between what \mention{cattle} licenses and what it rejects feels determinate, even though the underlying individuation variable is gradient.
...
Countability\ixsq{countability} confirms this picture. The \mention{folks} case is revealing: inter-speaker variation in the acceptability\ixsq{acceptability} of \mention{three folks} is real and robust. Some speakers place the boundary above \mention{folks} (it clears \mention{three}); others place it below (it doesn't). Each speaker's individual grammar has a determinate answer, but the community doesn't. The gradience\ixsq{gradience} shows up in population variance, not in graded individual judgments.
...
The predictions are \textit{gradient} by position in the hierarchy. Knowing that \mention{cattle} is quasi-count~-- that it accepts loose properties but rejects tight ones~-- lets you predict precisely which frames it will enter. The quasi-count pattern is projectible too. The category structure supports differentiated, not just all-or-nothing, induction.

Compare this to a merely nominal grouping~-- say, \enquote{nouns ending in -tion}. Knowing that \mention{nation} ends in -tion tells you nothing about its grammatical behaviour distinct from knowing it's a noun. The suffix doesn't support induction. Countability\ixsq{countability} does.
...
Third, it explains stability and instability together. Quasi-count nouns are stable because of functional anchoring\ixsq{functional anchoring}; \mention{folks} is unstable because it lacks anchoring. \mention{Pea} regularised because there was no anchor; \mention{data} is drifting because \mention{datum} is dying. The HPC\ixsq{HPC} account doesn't just describe which cases are stable~-- it explains why, and predicts which cases should be vulnerable to change.

The traditional question~-- \enquote{Is \mention{cattle} count or mass?}~-- obscures the underlying dynamics. The HPC question is: \enquote{Where does \mention{cattle} sit in the count basin\ixsq{basin}, and what's holding it there?} That question has an answer. The first one doesn't.
...
Here, the mechanism perturbed is the default basin\ixsq{basin} of attraction. Standard Average European languages like English force a binary choice: singular (unmarked) or plural (marked). This obliges speakers to default to the singular for singletons. But what if the default were different?
...
\textcite[532]{grimm2018} shows that Welsh base-form collectives behave very similarly to English quasi-count nouns: they accept loose properties (quantifiers like \mention{llawer} `much/many') but reject tight properties (numerals) without singulative\ixsq{singulative} marking. The \mention{cattle} profile isn't a quirk of English irregulars; it's a plausible attractor state for aggregate nouns when the pressure to act as a singular is removed. English \mention{cattle} can be read as a comparable pattern in a number-marking language.
...
\textcite{lima2014} argues that Yudja (Juruna family, Brazil) is a strong candidate for this state. In Yudja, numerals combine directly with notional mass nouns: \mention{\textit{txabïu apeta}} (`three blood') is grammatical and interpreted as `three drops/spots of blood'. The constraint that numerals demand atomic units appears to be relaxed or reweighted.
