...
To a lawyer, \mention{LEGO} is an adjective, a modifier that tracks a brand origin. To a six-year-old, a plastic brick is a discrete, bounded, manipulable object. And English has a deeply entrenched pattern for dealing with discrete, bounded, manipulable objects: it counts them. It adds an \mention{-s}. It makes them plural. When a child asks for \mention{three Legos}, they aren't making a mistake; they are applying an inference they've learned from thousands of similar cases. The count cluster is claiming the word.

This chapter is about that mechanism~-- the control loop that decides what gets counted. The count/mass distinction isn't a static binary. It's a dynamic negotiation. And as we'll see, the HPC framework we built in Part II handles this messy distinction cleanly.

Countability, I'll argue, isn't a definition. It's a homeostatic property cluster~-- but a peculiar one. Unlike the categories we've examined so far, countability isn't maintained by a single braid of mechanisms. It's a coupling between two distinct clusters~-- one semantic, one morphosyntactic~-- each maintained by its own mechanisms, each an HPC in its own right.

%~-- ~-- ~-- ~-- ~-- ~-- ~-- ~-- ~-- ~-- ~-- ~-- ~-- ~-- ~-- ~-- --
\section{One word, two categories}
\label{sec:9:two-categories}

The word \textit{countability} hides a structural fact. It names not a single category but a coupling between two: the \textbf{individuation cluster} (semantic) and the \textbf{count cluster} (morphosyntactic). Each is maintained by its own mechanisms. Countability is what emerges when they meet.
...
Individuation isn't a single property. It's a cluster of semantic properties that typically travel together:

\begin{itemize}
    \item \textbf{Boundedness}: The referent has discrete edges~-- a spatial or conceptual boundary that separates it from its environment.
    \item \textbf{Atomicity}: The referent is accessible as a unit~-- something that can be singled out, pointed to, tracked.
    \item \textbf{Enumerability}: The referent is compatible with exact counting~-- three of them, not just much of it.
    \item \textbf{Homogeneity resistance}: Parts of the referent are not the same kind as the whole. Half a cat is not a cat; half of water is still water.
\end{itemize}

These properties cluster. When a referent is bounded, it's usually atomic; when it's atomic, it's usually enumerable; when it's enumerable, it usually resists homogeneous subdivision. The clustering isn't accidental.
...
But edge detection isn't vision-specific. Blind echolocators produce tongue clicks and interpret the returning echoes to perceive objects. Research shows that they can identify object shape, size, and location with remarkable precision~-- and fMRI studies reveal that they activate visual cortex when doing so. The perceptual system has found a different input modality but the same computational goal: bounded individuals.

The pattern extends across species. Bats use sonar for prey localization; dolphins use echolocation for object recognition. Edge detection~-- the computational problem of finding where one thing ends and another begins~-- has evolved independently in multiple lineages, via multiple sensory modalities. This is the signature of strong functional pressure. The problem of individuating objects is so fundamental that perceptual systems converge on solutions to it.

\paragraph{Infant core knowledge.} \citet{spelke2007} argues that human infants are equipped with core knowledge systems for objects~-- systems that operate from the first months of life. Infants as young as four months perceive objects as cohesive (parts move together), bounded (edges mark identity), and continuous (objects persist through occlusion). These are the same properties that define the individuation cluster in adult conceptual systems. The mechanisms that maintain individuation are not learned from scratch; they're built on an early foundation.
...
\subsection{Individuation is gradient}

Although the properties cluster, they don't cluster perfectly. Some referents are more individuated than others.

A cat is maximally individuated: bounded, atomic, enumerable, homogeneity-resistant. A pile of sand is weakly individuated: no sharp edges, no natural atoms (or too many), compatible with homogeneous division. \mention{Cattle} denotes ontologically discrete animals, but the noun construes them as an aggregate~-- grammatically group-like, not atom-like.

The gradient nature of individuation is crucial for what comes next. The count cluster~-- the morphosyntactic syndrome~-- tracks individuation. When individuation is strong, the full count cluster applies. When it weakens, the cluster frays. But it frays in an orderly way.
...
\begin{itemize}
    \item \textbf{Denotation}: Discrete, bounded entities.
    \item \textbf{Pluralization}: It inflects for number (\mention{cats}).
    \item \textbf{Determinatives}: It takes exact cardinals (\mention{one cat}, \mention{two cats}) and fuzzy count quantifiers (\mention{many}, \mention{fewer}).
    \item \textbf{Agreement}: It triggers singular or plural verb agreement matching its morphology.
\end{itemize}

For a prototypical mass noun like \mention{water}:
\begin{itemize}
    \item \textbf{Denotation}: Undifferentiated substance.
    \item \textbf{Pluralization}: No plural form (*\mention{waters} is special, meaning `types of' or `bodies of').
    \item \textbf{Determinatives}: It rejects cardinals (*\mention{two waters}) and takes mass quantifiers (\mention{much}, \mention{less}).
    \item \textbf{Agreement}: Always singular.
\end{itemize}
...
The answer is \textbf{bidirectional inference}~-- the mechanism that couples semantic construal to grammatical form.
