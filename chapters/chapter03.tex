...
But none has made mechanism fully central. Usage-based approaches invoke frequency; construction grammar invokes analogy and inheritance; sociolinguistics invokes community transmission; functionalism invokes communicative need. Each names a mechanism, or a family of mechanisms. Recent work has begun to synthesise these insights: \textcite{schmid2020} models category maintenance as a feedback loop between cognitive entrenchment and social conventionalisation; \textcite{spike2020} argues explicitly that cross-linguistic categories are homeostatic property clusters in Boyd's sense, not essences but stable configurations; \textcite{dahl2016} applies the HPC framework to tense-aspect gram types. These are important precedents. What remains undeveloped is a systematic application to grammatical categories writ large~-- not just TAM or word-kinds but phonemes, constructions, word classes, and the cluster-of-clusters we call \enquote{the grammar}~-- together with explicit diagnostic criteria for distinguishing genuine kinds from taxonomic conveniences.
