...
A zipper doesn't have two parts; it has three. The teeth on each side are just parallel rows without the slider. The slider is what makes them engage.

Chapter~\ref{ch:projectibility} developed a triadic structure that maps directly onto this. Form is one row of teeth: the perceivable signal, the distributional properties. Object is the other: what the form is about, its contrastive identity. The interpretant~-- the inferential habit the sign generates~-- is the slider. Projectibility, recall, \emph{is} reliable interpretant generation. The slider is what the maintenance buys.

This is why the dyadic picture couldn't handle jams and mis-tracking. If coupling is just form-to-meaning, there's no room for the failure to be \emph{between} them. The interpretant gives you that room:
...
The regimes track a gradient in how much mediation the interpretant contributes. In transparent coupling, mediation is nearly automatic: form determines object, object determines interpretant, and the habit fires without deliberation. The sign functions almost as an index~-- brute connection with minimal interpretive work. In architectural coupling, the interpretant does genuine compositional work: it mediates between locally correct form--object pairings to produce a global meaning. The habit constitutes the relation rather than merely following from it. In loose coupling, mediation is underdetermined: multiple would-bes are available, and which one fires depends on common ground, context, and coordination. The gradient isn't a ladder of complexity; it's a map of where the would-be bears the load.
