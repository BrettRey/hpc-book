...
The three cases above~-- phonemes, words, constructions~-- aren't a ladder. They are regions in a stabilization space defined by how form couples to value and which stabilizers carry the load. Table~\ref{tab:13:stabilizer-map} summarizes the mapping.
...
Morphosyntax is the zone of maximum systematicity: the region of grammar where coupling is enforced, not optional.

We can actually measure this tightness. \textcite{Reynolds2026} quantifies the \term{packaging tightness} of the determiner--head relationship in English~-- the link that LBE fails to break. Using a dependency locality metric, he finds that determiners and their heads ($D \to N$) exhibit a packaging score of $k=4$ (extremely tight). They are resistant to interruption by parentheticals, adverbs, or hesitation markers to a degree that statistical independence cannot explain. The system treats $D+N$ not just as neighbors, but as a rigid transmission unit. This is what obligatory composition looks like in the data: a coupling so strong that the elements refuse to be separated.

Chapter~\ref{ch:grammaticality-itself} asks what happens when we take this observation seriously. If grammaticality is what emerges when form--value coupling is obligatory, compositional, and learnable~-- then grammaticality itself is a kind.
