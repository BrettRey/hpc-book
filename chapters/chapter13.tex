...
Evidence comes from PHOIBLE 2.0 \parencite{moran2019phoible}, a database of phoneme inventories for roughly 2,700 languages. Two patterns satisfy the diagnostics. Plotting kernel-density ridgelines of total inventory sizes by family reveals a \emph{stability band}: medians cluster between 20 and 50 segments across unrelated families, with thin tails beyond. This isn't an artifact of pooling; it's cross-family regularity enabling inventory-level projection. Meanwhile, the vowel /y/~-- a front rounded vowel requiring precise articulatory coordination~-- shows a \emph{scaling curve}: a logistic model predicting /y/-presence from vowel-inventory size (cross-validated, grouped by family) discriminates effectively between inventories that have the sound and those that don't (Area Under the Receiver Operating Characteristic curve, ROC-AUC $\approx 0.70$, well above the 0.50 baseline of chance guessing). This predictive power confirms that the biological constraints of articulation objectively shape the inventory even as individual sounds shift. Marked segments lacking quantal robustness appear mainly in larger systems where there's acoustic room. This is exactly what the stabilizer story predicts.
...
If the framework is correct, we should also find a \emph{null case}: a robust semantic pole that simply never develops a morphosyntactic coupling in a given language. Consider numerical classification in Mandarin. The semantic concept of individuation exists~-- speakers count things~-- but it does not grammaticalize into a forced morphosyntactic choice on every noun phrase the way countability does in English. Mandarin speakers use classifier constructions (e.g., \mention{yī gè rén} `one \textsc{cl} person'), but these classifiers are lexical items that group nouns by shape or function, not an abstract grammatical toggle enforced on the noun itself. The semantic pole exists, but the grammatical zipper never engaged. When coupling fails to develop, the semantic pressures are absorbed by lexical or constructional alternatives. The coupling continuum spans all the way to zero.
