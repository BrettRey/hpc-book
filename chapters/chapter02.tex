...
This isn't carelessness. \textit{CGEL} is a work of exceptional rigour, two decades in the making, with scrupulous attention to just the kind of definitional precision that would flag this inconsistency. If this grammar produces an incoherence this stark, the problem lies not with the grammarians but with something in the method itself.
...
Aristotle's essentialism was more sophisticated than the methodological practice I'm diagnosing. For Aristotle, an essence isn't a checklist of necessary and sufficient conditions but an explanatory core: the \emph{to ti \^en einai}, what it is to be the thing, which explains why the thing has the properties it has. A tiger's essence, on this view, is whatever makes it do tiger-things~-- not a list of features (stripes, carnivory, size) but a causal-explanatory principle. Nor did Aristotle assume that every category has an essence. Artefacts, social kinds, arbitrary groupings~-- these aren't essence-bearing in the way that natural kinds are. The question of \emph{which} categories are natural kinds was live, not presupposed.

What I'm calling essentialism is a methodological descendant~-- but not a direct one. Through Scholastic systematisation, Locke's nominal essences, logical positivism's verification conditions, and mid-century conceptual analysis, the Aristotelian framework hardened into the checklist picture: necessary and sufficient conditions as the mark of genuine understanding. \textcite{joos1957} put it with characteristic bluntness: \enquote{The linguistic categories, then, are absolutes which admit of no compromise.} Anything that couldn't be captured by \enquote{a finite number of absolute categories} was deemed non-linguistic and expelled from analysis.

Structuralism complicates this narrative. Saussure's insistence that linguistic units are purely differential~-- \emph{dans la langue il n'y a que des différences}~-- is not checklist essentialism; categories are constituted by systemic relations, not intrinsic properties. The Prague School phoneme was defined oppositionally, by contrasts, not by bundles of inherent features. But what survived into practical descriptive work was the determinacy assumption: that for any item and any category, there is a fact of the matter whether the item belongs. The relational metaphysics didn't translate into tolerance for indeterminate membership. By the time category-assignment became routine practice in the great descriptive grammars, the method presumed sharp boundaries even where the theory might have licensed fuzziness.

The distinction matters because the alternative I develop in Chapter~5~-- homeostatic property cluster theory~-- is sometimes called neo-Aristotelian. It recovers real natural kinds with genuine explanatory structure, just not checklist-definitional structure. The homeostatic mechanisms that maintain a category \emph{are} its essence, causally understood.
...
Regimentation offers a middle path, but a modest one. Carnap's project, shorn of its analyticity assumptions, survives as a pragmatic method: take vague ordinary concepts, construct precise replacements, evaluate by whether they support the generalisations you need. \textit{CGEL}'s \term{canonical clause}~-- finite, with overt subject and declarative syntax~-- is a benchmark for measuring non-canonical cases, not a discovery claim. Operationalised categories in corpus annotation work the same way: stipulate criteria, apply consistently, use the results. These are good tools; the error is mistaking the tool for the thing.

\bigskip

There are corners of linguistics where essentialism isn't a temptation but a job requirement.

Consider lexicography. Dictionary editors face practical constraints that reward essentialist form: entries must be short, portable, scannable. A definition that says \enquote{it depends on multiple factors that interact in context-sensitive ways} doesn't fit in a dictionary. The format demands necessary and sufficient conditions, or at least clean prototype descriptions with bounded polysemy. Part-of-speech labels must be discrete: P or A, not \enquote{more P-like in these constructions and more A-like in those.}

This pressure produces a distinctive pathology. The metalanguage defines categories one way; the labelling practice violates the definitions. English dictionaries routinely define prepositions as words that take noun-phrase complements~-- then label \mention{near}, \mention{worth}, \mention{like}, and \mention{despite} as prepositions even when their complement possibilities diverge wildly from the definition \autocite{Reynolds2025}. The practice is internally inconsistent, but the inconsistency is invisible to users who look up one word at a time. The dictionary works as a tool. It fails as an ontology.

Lexicographers aren't incompetent~-- the format demands essences the language doesn't supply. Lexicography reveals what happens when essentialist architecture is imposed on natural-language categories by practical necessity: local coherence, global inconsistency, and quiet violations of stated principles.
...
Law occupies similar territory. A \term{felony} is whatever the criminal code says it is; \term{negligence} is defined by statute and refined by precedent. When boundaries blur~-- is computer hacking a form of trespass?~-- courts stipulate. That's what landmark cases do. \citet{hart1961} called this \term{open texture}: the penumbral region where existing rules don't determine outcomes. But Hart's point was that open texture is a \emph{problem} for law, not a feature. Legal systems respond to indeterminacy with institutional repair: litigation, legislation, constitutional amendment. The pressure is toward definitional clarity. Natural language has no Supreme Court. Grammaticalisation proceeds without appeal.
...
\begin{table}[htbp]
    ...
    \begin{tabular}{@{}lcccc@{}}
        ...
        & \makecell{Diachronic\\continuity} 
        & Operationalisability \\
        \midrule
        \textit{bachelor}, \textit{prime number} & ✓ & ✓ & ✓ & ✓ \\
        Canonical nouns, phonemes                & ✓ & ✓ & ✓ & ✓ \\
        \textit{fun}, \textit{near}, \textit{otherwise} & \textasciitilde & \textasciitilde & \textasciitilde & ✗ \\
        \textit{will}, \textit{going to}         & ✗ & \textasciitilde & ✗ & \textasciitilde \\
        Verbless clauses                         & ✗ & \textasciitilde & ✓ & ✗ \\
        \textit{subject} (cross-linguistically)  & ✗ & \textasciitilde & \textasciitilde & ✗ \\
        \bottomrule
    \end{tabular}
...
Phonology exhibits the same pattern. What's a phoneme?
...
The grammar provides two criteria for clausehood: a syntactic one in Chapter~\ref{ch:words-wont-hold-still}, where clauses are VP-headed and identified by their verbs, and a semantic one in Chapter~\ref{ch:what-changes}, where clauses exhibit subject + predicate organization.
...
The global inconsistency only becomes visible when you step back and ask: what is the essence of clausehood? VP-headedness, or subject--predicate structure? If VP-headedness, then verbless clauses aren't clauses. If subject--predicate structure, then Chapter~\ref{ch:words-wont-hold-still}'s identification criterion is wrong. Essentialism requires a single answer. \textit{CGEL} provides two, deploying whichever one serves the immediate analysis. The question \textit{CGEL} can't ask is: why do VP-headedness and subject--predicate structure usually coincide?
...
Usage-based and variationist traditions have begun asking this second question. Bybee's work on frequency and entrenchment, Goldberg's on constructional networks, Labov's on the social embedding of variation~-- all attend to the mechanisms that produce and maintain linguistic patterns. But the ontological stakes often remain implicit. The mechanisms are treated as causes of category behaviour rather than as constitutive of what categories are. Chapter~\ref{ch:kinds-without-essences} makes the ontological claim explicit: the mechanisms don't just \emph{produce} the convergence; they \emph{are} the category.
...
Essentialism can't ask this question, because essentialism presupposes the convergence rather than explaining it. If \term{noun} is defined by property P, then every noun has P by definition; the convergence with properties Q and R is either accidental or stipulated. The framework makes convergence invisible as an explanandum.

Boundary cases~-- \mention{furniture}, verbless clauses, near-mergers, \term{subject} in ergative languages~-- are where criteria come apart. Under essentialism, these are anomalies. But what exactly is anomalous? Two things, and they are different.

The first is indeterminacy: the framework can't deliver a yes/no answer. Is this a clause or not? The question seems to demand an answer, and none is forthcoming. Essentialism treats this as a problem to be solved~-- find the right criterion and the answer will emerge. HPC treats it as a prediction: where mechanisms compete, membership is genuinely unsettled. The second is a failure of essence: different candidate essences (VP-headedness, subject-predicate structure) pick out different extensions, and the framework provides no way to adjudicate.

These failures are connected but not identical. The connection is internal to essentialism: if essence grounds determinacy, then when candidate essences diverge, determinacy fails. But this is a framework-internal consequence, not a logical necessity. A framework that grounds determinacy differently~-- in causal structure rather than necessary and sufficient conditions~-- might recover determinate membership without essences.

For now, essentialism treats boundary cases as problem cases to be filed away with special labels, exceptions that prove (in the old sense of \mention{test}) the rule. They generate debates about which criterion is really essential. They don't generate inquiry into the mechanisms that usually keep criteria aligned.
...
The essentialist search for definitions doesn't merely fail to find definitions. It prevents the question that boundary cases could answer.


Essentialism treats convergence as the default state of nature: categories just \emph{are} determinate, so of course their properties align. Divergence is the anomaly. But in a complex, evolving system like language, convergence is the achievement. The fact that distributional, semantic, and morphological properties cluster tightly enough to make the notion \term{noun} useful for 95\% of the vocabulary is a massive statistical coincidence that demands explanation. Essentialism can't explain it because it presupposes it.

Once you treat convergence itself as something to be explained, you are asking the question that \textcite{lakoff1987} asked of cognitive categories and that homeostatic property cluster theory was built to answer. Lakoff showed that categories in general~-- not just linguistic ones~-- resist classical definition: they exhibit prototype effects, family resemblance structure, and radial organisation around central members. This is an empirical critique: essentialism fails the tests it should pass.
...
This pattern will be familiar to philosophers of biology~-- and to syntacticians who've watched the island debate.
...
The diachronic problem recurs too. Asking at what precise moment a lineage ceased to be a dinosaur and became a bird is like asking when \mention{will} became an auxiliary. The question demands a boundary that the process of evolution does not supply.
...
Publication norms reward definitional disputes. \textcite{postal1966} titled his paper \enquote{On so-called \enquote{pronouns} in English}~-- scare quotes announcing that pronouns aren't really pronouns but determiners preceding silent nouns. The paper launched decades of debate, culminating in Abney's DP hypothesis \autocite{abney1987}~-- an unpublished dissertation that nonetheless reshaped the field, less through empirical vindication than through perceived endorsement. A paper arguing that X is really Y~-- that raising verbs are really control verbs, that determiners are really functional heads, that the passive is really a species of unaccusativity~-- has a clear contribution: it advances a position in a debate. A paper arguing that the X/Y boundary is gradient and maintained by multiple mechanisms has a murkier contribution. It sounds like giving up. The rhetoric of \enquote{really} signals progress; the rhetoric of \enquote{it depends} signals inconclusiveness. Tenure committees notice. The \enquote{is X really Y} literature could furnish a small library. The library's classification system would be disputed.
...
A final clarification. Nothing in this chapter denies that sentences have structure or that categories are real. The intuition that \textit{The dog bit the man} differs from \textit{The man bit the dog}, and that \textit{dog} and \textit{table} cluster together~-- these are correct. What the chapter denies is the essentialist account of what makes them true. Structure is real~-- causal-historical, not definitional.

The choice isn't essence or chaos. If the essentialist says categories are defined, and is wrong, it doesn't follow that categories are arbitrary. Categories may be real but not immutable, revisable but not arbitrary, maintained by mechanisms rather than constituted by essences.

The verbless clauses remain. \textit{CGEL}'s definitions are still incoherent. The framework that trained you can't explain its own categories. What it can do~-- what nominalism can't~-- is point toward what an explanation would require: not better definitions, but an account of the mechanisms that hold the cluster together even when one property is missing. That's only half the story. You also need to explain why the cluster has the shape it has~-- why VP-headedness bundles with predication, why auxiliaries cluster the properties they do. That's an etiological question: what selective pressures, operating over the history of transmission, shaped these categories to serve these functions? Chapter~\ref{ch:kinds-without-essences} takes up both questions.

Much of what that chapter will draw on already exists. Variationists have the transmission dynamics. Cognitive linguists have entrenchment and constructional schemas. Grammaticalisation theorists have the diachronic trajectories. Typologists have the cross-linguistic pressures. What's missing isn't the mechanisms but the ontological frame: the explicit claim that these mechanisms don't merely \emph{cause} categories to behave as they do but \emph{constitute} what categories are. The next chapter surveys traditions that have glimpsed this possibility without quite arriving at it.
