...
The distinction matters because the alternative I develop in Chapter~\ref{ch:kinds-without-essences}~-- \ixs{homeostatic property cluster}homeostatic property cluster theory~-- is sometimes called neo-Aristotelian. It recovers real natural kinds with genuine explanatory structure, just not checklist-definitional structure. The homeostatic mechanisms that maintain a category \emph{are} its essence, causally understood.
...
\begin{table}[htbp]
    ...
    \begin{tabular}{@{}lcccc@{}}
        ...
        \midrule
        \textit{bachelor}, \textit{prime number} & \ipa{✓} & \ipa{✓} & \ipa{✓} & \ipa{✓} \\
        Canonical nouns, phonemes                & \ipa{✓} & \ipa{✓} & \ipa{✓} & \ipa{✓} \\
        \textit{fun}, \textit{near}, \textit{otherwise} & \textasciitilde & \textasciitilde & \textasciitilde & \ipa{✗} \\
        \textit{will}, \textit{going to}         & \ipa{✗} & \textasciitilde & \ipa{✗} & \textasciitilde \\
        Verbless clauses                         & \ipa{✗} & \textasciitilde & \ipa{✓} & \ipa{✗} \\
        \textit{subject} (cross-linguistically)  & \ipa{✗} & \textasciitilde & \textasciitilde & \ipa{✗} \\
        \bottomrule
    \end{tabular}
...
Once you treat convergence itself as something to be explained, you are asking the question that \ixn{Lakoff, George}\textcite{lakoff1987} asked of cognitive categories and that homeostatic property cluster theory was built to answer. Lakoff showed that categories in general~-- not just linguistic ones~-- resist classical definition: they exhibit prototype effects, family resemblance structure, and radial organisation around central members. This is an empirical critique: essentialism fails the tests it should pass.
