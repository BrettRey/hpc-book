...
\begin{table}[htbp]
    ...
    \begin{tabular}{@{}lcccc@{}}
        ...
        \midrule
        \textit{bachelor}, \textit{prime number} & $\checkmark$ & $\checkmark$ & $\checkmark$ & $\checkmark$ \\
        Canonical nouns, phonemes                & $\checkmark$ & $\checkmark$ & $\checkmark$ & $\checkmark$ \\
        \textit{fun}, \textit{near}, \textit{otherwise} & \textasciitilde & \textasciitilde & \textasciitilde & $\times$ \\
        \textit{will}, \textit{going to}         & $\times$ & \textasciitilde & $\times$ & \textasciitilde \\
        Verbless clauses                         & $\times$ & \textasciitilde & $\checkmark$ & $\times$ \\
        \textit{subject} (cross-linguistically)  & $\times$ & \textasciitilde & \textasciitilde & $\times$ \\
        \bottomrule
    \end{tabular}
...
For canonical nouns, morphological, syntactic, and semantic criteria all pick out the same set. That is not a trivial convenience; it's a fact about how linguistic structure is packaged. What keeps those properties bundled together strongly enough that \term{noun} is useful for most of the lexicon? And what happens in the boundary zone where the bundling weakens?
...
The verbless clauses remain. \textit{CGEL}'s definitions are still incoherent. What the incoherence points to is not the need for better definitions but for a different explanatory target: an account of why the familiar criteria usually line up, and why they sometimes come apart.
