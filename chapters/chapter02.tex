...
The nativist response is to ground competence in biology: Universal Grammar is genetically specified, so the formal properties are inherited rather than learned. But this appeal points in the wrong direction. Biology is precisely the domain where essentialism was tested and found wanting: species aren't defined by genetic essences but maintained by mechanisms of reproduction, selection, and development. If competence is biological, that's a reason to expect homeostatic structure, not a rescue of essential definitions.
...
These three approaches~-- behavioural cluster, configurational position, syntactic pivot~-- don't pick out the same set of NPs across languages. In Dyirbal, the absolutive argument controls coordination deletion; by Dixon's criterion, it's the subject. By the configurational criterion, the ergative argument might be the subject, depending on your phrase structure assumptions. By Keenan's cluster, neither argument has a clear majority of subject properties. The criteria aren't just different; they operate at different levels. Keenan's properties are partly behavioural (what subjects do in discourse), the configurational definition is structural (where subjects appear in phrase structure), and Dixon's criterion is functional (what role subjects play in cross-clausal operations). Asking which is the real definition conflates ontological levels that may not align.
...
The question that doesn't arise is: \textit{Why do these criteria converge in the first place?} The first question assumes the category and searches for its boundary. The second treats the category itself as the thing to be explained.
...
Once you treat convergence itself as something to be explained, you are asking the question that homeostatic property cluster theory was built to answer. This is an empirical critique: essentialism fails the tests it should pass.
