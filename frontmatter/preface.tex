...
Linguistic examples~-- cases where the form itself is under discussion~-- are italicized: \mention{cattle}, \mention{the}. Category labels and technical terms appear in small caps: \textsc{noun}, \textsc{verb}, \term{projectibility}. The glossary at the back collects key terms with definitions, mechanisms, diagnostics, and cross-references.

Unacceptable examples are marked with a superscript asterisk (*); marginal or disputed examples with a superscript question mark (\textsuperscript{?}). Page references to \textit{CGEL} are given where relevant.
