#!/usr/bin/env python3
"""Strict second-pass prune for specific index tag types.

Reads an existing book-indexer judgment report, filters to kept occurrences for
selected tag types (for example: name, language), and asks an LLM to re-judge
those occurrences under stricter criteria.
...
def build_prompt(batch: list[dict[str, Any]]) -> tuple[str, str]:
    system = (
        ...
        "for NAME and LEXICAL indexes. "
        "Rules:\n"
        "1) Name index (type=name): keep only when the person's ideas, claims, "
        "methods, or contrasts are substantively discussed. Drop passing mentions, "
        "mere citation-like name checks, and rhetorical name drops.\n"
        "2) Lexical index (type=language): keep only when the lexical item itself "
        "is analyzed as data (form, meaning, category behavior, historical change, "
        "contrast, diagnostics). Drop incidental ordinary usage, throwaway examples, "
        "or repeated mentions that add no new discussion.\n"
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        help="Tag types to strict-prune (default: name language)",
    )
    parser.add_argument(
        "--llm-bridge",
    ...
    parser.add_argument("--chunk-size", type=int, default=60, help="Batch size")
    ...
    include_types = {t.strip().lower() for t in args.types if t.strip()}
    ...
    print(f"Types: {sorted(include_types)}")
        ...
        for i, batch in enumerate(batches, start=1):
            ...
            if result is None:
                ...
                snapshot = {
                    "items": target_items,
                    "decisions": [
                        strict_decisions[int(it["id"])]
                        for it in target_items
                        if int(it["id"]) in strict_decisions
                    ],
                    "notes": notes,
                    "meta": {
    ...
    kept = sum(1 for d in final["decisions"] if d.get("keep") is True)
    dropped = sum(1 for d in final["decisions"] if d.get("keep") is False)
    print(f"Wrote {out_path}")
    print(f"Decisions: kept={kept}, dropped={dropped}, total={len(final['decisions'])}")
    return 0
