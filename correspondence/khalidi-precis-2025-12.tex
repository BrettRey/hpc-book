...
\begin{itemize}
    ...
    \item \textbf{Entrenchment}: High-frequency items anchor categories; low-frequency items cluster around them, drawn by analogical pressure \citep{bybee2006}.
...
The framework crystallises in a slogan: \textbf{A category is a profile, stabilised by mechanisms, projectible relative to purposes.} Three parts, three levels: the observable surface (profile), the metaphysics (what maintains it), and the epistemology indexed to pragmatics (what inferences the category supports, for whom). The slogan is a mnemonic, not a substitute for the argument~-- it can't tell you which mechanisms matter or when projectibility fails. But it guards against the confusions that have plagued the literature.
...
``Projectibility'' in Goodman's sense is the capacity of a predicate to support inductive inference. \term{Green} is projectible; \term{grue} isn't. Your own work emphasizes projectibility as a hallmark of natural kinds: ``The main reason why we should be interested in natural categories,'' you write, ``is that they allow us to project from one case to another'' \citep[ch.~1]{khalidi2013}.
...
Not every category label picks out a natural kind. The book's framework (Chapter~8, ``Failure Modes'') provides criteria for distinguishing real HPCs from taxonomic conveniences.
...
\textbf{Fat clustering}: Some categories lump together what are actually distinct HPCs. The traditional category \term{adverb} is the classic case~-- a wastebasket that includes ``manner adverbs,'' ``sentence adverbs,'' ``degree modifiers,'' ``focusing adverbs,'' and ``connective adverbs.'' What these share is a negative definition: they're not nouns, verbs, adjectives, or prepositions. The properties don't cluster; the mechanisms don't align; the category doesn't project.

\textbf{Negative diagnosis}: Some categories exist only as taxonomic residue. \term{Non-finite} picks out verb forms that lack tense marking, but the class includes infinitives, gerunds, and participles~-- forms that share almost nothing else. The category is useful for stating what something isn't, not for predicting what it does.

This connects to your own treatment of ``payoff similarity'' versus ``property similarity.'' A category can seem useful~-- it groups things that matter for some purpose~-- without constituting a natural kind. NPIs matter for semantic licensing, but that's payoff similarity. The properties don't cluster homeostatically; the category doesn't support induction beyond the licensing facts that defined it.
...
\textbf{Definiteness and deitality} (Chapter~10, notes): The article \mention{the} marks a morphosyntactic property I call \term{deitality}: restriction with \term{there}-insertion, compatibility with partitive \term{of}, incompatibility with \term{one}-substitution. This cluster is distinct from semantic \term{definiteness} (identifiability, uniqueness, familiarity). Weak definites (``the hospital,'' ``the bus'') belong to the morphosyntactic cluster but not the semantic one. The literature's ``exceptions'' dissolve once you recognise that two HPCs are in play.

\textbf{Word classes} (Chapter~11, notes): The noun--verb contrast is crosslinguistically stable because both semantic mechanisms (reference vs.\ predication) and morphosyntactic mechanisms (distribution, morphology) reinforce it. Adjectives vary across languages because the mechanisms don't align as tightly: some languages encode property concepts as nouns, others as verbs, others as a distinct class. The framework predicts that category stability tracks mechanism alignment.
...
More specifically, the framework counsels linguists to \textbf{always specify which field their category projects in}. If your category is syntactic, test it with syntactic diagnostics; don't assume that a semantic characterisation will project to distribution. If your category is semantic, don't be surprised when it fails to align perfectly with a morphosyntactic cluster. The slippage isn't noise; it's evidence of distinct HPCs.

%~-- ~-- ~-- ~-- ~-- ~-- ~-- ~-- ~-- ~-- ~-- ~-- ~-- ~-- ~-- ~-- ~-- ~-- ~-- ~-- ~-- ~-- ~-- --
\section{Current Status and Invitation}
%~-- ~-- ~-- ~-- ~-- ~-- ~-- ~-- ~-- ~-- ~-- ~-- ~-- ~-- ~-- ~-- ~-- ~-- ~-- ~-- ~-- ~-- ~-- --

The book is roughly two-thirds drafted. Part I (The Problem: Chapters 1--3) and Part II (The Fix: Chapters 4--8) are complete in prose. Part III (Categories Reconsidered: Chapters 9--12) has one chapter drafted (Countability) and three in detailed notes (Definiteness, Word Classes, The Stack). Part IV (Implications: Chapters 13--14) exists as a theoretical framework document.
