% Draft section for Chapter 5: The Discreteness Problem
% This section bridges HPC theory with a formal account of how 
% mechanism-maintained clusters yield discrete-ish boundaries.
% 
% Draws on: Reynolds (2025) "A Hyperreal Model of Sorites Tolerance"
% 
% Position in chapter: After introducing HPC from Boyd/biology,
% before discussing specific linguistic mechanisms.

\section{How clusters become discrete}
\label{sec:5:discreteness}

The homeostatic property cluster framework explains why categories persist: mechanisms hold properties together. But it leaves a question unanswered. If the underlying properties are gradient~-- if frequency distributions are continuous, if phonetic realizations vary along continuous dimensions, if semantic features shade into one another~-- how do discrete-ish categories emerge? Why isn't everything fuzzy all the way down?

This is the discreteness problem. It arises for any account that rejects essences. The essentialist has a ready answer: categories are discrete because their defining properties are discrete. Either something has the essence or it doesn't. The prototype theorist has no answer at all~-- gradience is just how things are. The HPC theorist needs a third option: an account of how mechanisms operating on continuous substrates produce categorical outcomes.
...
The hyperreal framework extends naturally. Replace the single dimension of grain count with a multi-dimensional feature space. Replace the single predicate $P$ with a family of predicates $P_1, \ldots, P_n$ corresponding to different categories. Replace the linear Sorites series with a space of possible items, each located at some point in feature space.
...
This is not evasion. There is a principled way to construct such metrics for heterogeneous feature spaces. The strategy, familiar from statistical ecology and clustering analysis, proceeds in two steps \citep[cf.][]{gower1971general}.
...
This construction is a normed, slightly more structured cousin of Gower distance, widely used for mixed-type data \citep{gower1971general}. It is also in the spirit of Gärdenfors's \emph{conceptual spaces}: multiple quality dimensions grouped into domains, each with its own metric, combined via weighted aggregation across domains \citep{gardenfors2000conceptual}. The connection to HPC is direct: the weights $w_k$ encode which properties play a stronger homeostatic role in stabilizing the category. Properties central to the cluster get large weights; free riders and peripheral features get small ones.

I am not claiming this construction is uniquely correct. The point is that there exists a class of admissible metrics that yield the same small-change structure, and any member of that class makes precise the idea that some multi-dimensional perturbations are negligible relative to the current scale. The Relative Tolerance condition holds for all admissible metrics, not just one distinguished choice.

\paragraph{Visualizing the landscape.}
The three-dimensional surface we have been imagining is a visualization: the potential function $V$ is defined over the full feature space $X \subseteq \mathbb{R}^n$, with category cores corresponding to local minima and boundaries to the stable manifolds of saddle points. Any particular ``valley and ridge'' picture is the graph of $V$ over a low-dimensional slice of $X$ on which the relevant contrasts are visible. Choosing the metric~-- the weights $w_k$ and the embedding functions $\phi_k$~-- is, in effect, choosing a geometry for the energy landscape: which directions are steep, which are shallow, which features matter most for which basin. Relative Tolerance then says: stay within one valley when perturbations are small in that geometry.
...
\item Borderline cases distribute across the middle, because items near the boundary are sensitive to noise~-- small processing fluctuations can shift judgments in either direction.
  \item The gradient spread is widest near the boundary, not because grammaticality is gradient, but because that's where the signal-to-noise ratio is lowest.
\end{itemize}
...
Where the mechanisms align, the basins are disjoint: nouns and verbs occupy separate regions because the mechanisms that maintain them pull in different directions. Where the mechanisms cross, the basins overlap: adjectives and prepositions share enough properties~-- predicative function, modification of nominals~-- that some items cluster with both.
